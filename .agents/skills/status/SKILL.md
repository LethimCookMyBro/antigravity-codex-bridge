...
Summary: Display agent and project status. Progress tracking and status board.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `status` or the folder's specialized domain
...
Inspect the repo root and collect:

- project name and detected stack
- current preview status
- whether a plan file exists
- validation or test status if it is already available
...
→ See [shared preview status helper](../_shared/common-commands.md#preview-status)

## Fallbacks

- If preview is not running, show `stopped` instead of guessing a URL
- If the stack cannot be detected, report `Generic/unknown` and list the files checked
- If agent progress is unavailable, replace it with a `Worktree Snapshot` section
...
Use when you need a stable starting point before deeper status review.
```bash
# Create the output directory for this skill
mkdir -p status-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > status-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > status-output/scope-note.txt
```
...
head -30 status-output/artifact-manifest.txt | xargs -r sha256sum > status-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > status-output/review-start-utc.txt
date > status-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > status-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' status-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 status-output/signal-hits.txt | sort | uniq -c | sort -nr > status-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > status-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > status-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' status-output/extension-summary.txt
```
...
git status --short > status-output/git-status.txt 2>/dev/null || true
git diff --stat > status-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > status-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > status-output/status-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > status-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' status-output/status-report-starter.md
```
...
find status-output -maxdepth 2 -type f | sort > status-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf status-output/status-bundle.tgz status-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf status-output/status-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 status-output/signal-hits.txt | sort | uniq -c | sort -nr > status-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' status-output/review-queue.txt > status-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' status-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > status-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> status-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' status-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > status-output/status-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > status-output/status-final.md
# Preview the output files for QA
sed -n '1,20p' status-output/status-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > status-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 status-output/status-bundle.tgz > status-output/status-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > status-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' status-output/review-queue.txt > status-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > status-output/permission-limited-files.txt
```
...
find status-output -maxdepth 2 -type f | sort > status-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > status-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' status-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > status-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > status-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > status-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > status-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > status-output/current-utc.txt
```
...
- produce: `status-output/status-report.md`
- produce: `status-output/output-manifest.txt`
- produce: `status-output/status-bundle.tgz`
- produce: `status-output/status-findings.csv`
...
- Confirm `status-output` contains the report, manifest, and at least one evidence artifact.
