...
python_min: 3.8
description: Authorized IoT and OT exposure review for owned devices and industrial endpoints. Focus on identification, protocol inventory, and hardening evidence.
allowed-tools: Read, Glob, Grep, Bash
---
# SKILL: IoT and OT Exposure Review

Summary: Authorized IoT and OT exposure review for owned devices and industrial endpoints. Focus on identification, protocol inventory, and hardening evidence.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.

## WHEN TO USE THIS SKILL

- Use when the target is an IoT, embedded, firmware, or industrial endpoint in owned scope
- Use when the task is to inventory exposed services, firmware clues, and vendor surface
- Use when the user mentions PLCnext, Modbus, OPC UA, firmware images, or industrial gateways

## INPUT FROM

- hostname or IP of owned device
- firmware image path
- vendor and model notes

## KEY TECHNIQUES & TOOLS

### Phase 1: Passive and low-noise identification

```bash
# WARNING: live OT/IoT review must stay read-only and limited to owned or explicitly authorized assets
dig plc.example.local +short
curl -isk https://plc.example.local/ | sed -n '1,60p'
python3 .agents/scripts/h4cker_tls_cert_audit.py plc.example.local
```

### Phase 2: Service inventory in owned scope

```bash
# WARNING: run service inventory only in approved maintenance windows for production OT assets
nmap -Pn -sT -sV -p 80,443,1962,4840 plc.example.local
```

- Review:
  - HTTP/HTTPS admin surfaces
  - vendor banners
  - OPC UA exposure
  - TLS issuer and SANs
  - firmware or product version clues

### Phase 3: Firmware or file triage

```bash
file firmware.bin
strings -a firmware.bin | head -120
binwalk firmware.bin | head -80
```

### Decision rules

- Do not run destructive industrial protocol actions or exploit checks against live control equipment
- If the task asks to scan for vulnerabilities on a PLC or OT asset, limit the work to version/exposure inventory and defensive review unless the user provides an offline lab artifact
- If the artifact is firmware, prefer offline review over live interaction
- If the target is safety-critical, state that active validation must stay read-only and maintenance-window approved

## Fallbacks

- If the vendor or model is unknown, identify it from banners, certificates, or firmware strings before making protocol assumptions
- If industrial tooling like `binwalk` or `nmap` is unavailable, stay with `file`, `strings`, `curl`, and certificate review
- If the asset is production-critical, return a maintenance-window note before any active service inventory
...
- Input to `exploit-development`:
  ...
  - suspected vulnerable component
  - vendor/version evidence

- Input to `methodology`:
  - constraints for future assessment phases
...
Use when you need a stable starting point before deeper iot-hacking review.
```bash
# Create the output directory for this skill
mkdir -p iot-hacking-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > iot-hacking-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > iot-hacking-output/scope-note.txt
```
...
head -30 iot-hacking-output/artifact-manifest.txt | xargs -r sha256sum > iot-hacking-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > iot-hacking-output/review-start-utc.txt
date > iot-hacking-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > iot-hacking-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' iot-hacking-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 iot-hacking-output/signal-hits.txt | sort | uniq -c | sort -nr > iot-hacking-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > iot-hacking-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > iot-hacking-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' iot-hacking-output/extension-summary.txt
```
...
git status --short > iot-hacking-output/git-status.txt 2>/dev/null || true
git diff --stat > iot-hacking-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > iot-hacking-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > iot-hacking-output/iot-hacking-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > iot-hacking-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' iot-hacking-output/iot-hacking-report-starter.md
```
...
find iot-hacking-output -maxdepth 2 -type f | sort > iot-hacking-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf iot-hacking-output/iot-hacking-bundle.tgz iot-hacking-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf iot-hacking-output/iot-hacking-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 iot-hacking-output/signal-hits.txt | sort | uniq -c | sort -nr > iot-hacking-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' iot-hacking-output/review-queue.txt > iot-hacking-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' iot-hacking-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > iot-hacking-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> iot-hacking-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' iot-hacking-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > iot-hacking-output/iot-hacking-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > iot-hacking-output/iot-hacking-final.md
# Preview the output files for QA
sed -n '1,20p' iot-hacking-output/iot-hacking-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > iot-hacking-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 iot-hacking-output/iot-hacking-bundle.tgz > iot-hacking-output/iot-hacking-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > iot-hacking-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' iot-hacking-output/review-queue.txt > iot-hacking-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > iot-hacking-output/permission-limited-files.txt
```
...
find iot-hacking-output -maxdepth 2 -type f | sort > iot-hacking-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > iot-hacking-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' iot-hacking-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > iot-hacking-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > iot-hacking-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > iot-hacking-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > iot-hacking-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > iot-hacking-output/current-utc.txt
```
...
- produce: `iot-hacking-output/iot-hacking-report.md`
- produce: `iot-hacking-output/output-manifest.txt`
- produce: `iot-hacking-output/iot-hacking-bundle.tgz`
- produce: `iot-hacking-output/iot-hacking-findings.csv`
...
- Confirm `iot-hacking-output` contains the report, manifest, and at least one evidence artifact.
