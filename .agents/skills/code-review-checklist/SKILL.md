...
version: 1.0.0
source: codex/code-review-checklist
...
Summary: Code review guidelines covering code quality, security, and best practices.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `code-review-checklist` or the folder's specialized domain
...
```bash
mkdir -p code-review-checklist-output
pwd > code-review-checklist-output/pwd.txt
rg --files . | head -200 > code-review-checklist-output/repo-files.txt
git status --short > code-review-checklist-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper code-review-checklist review.
```bash
# Create the output directory for this skill
mkdir -p code-review-checklist-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > code-review-checklist-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > code-review-checklist-output/scope-note.txt
```
...
head -30 code-review-checklist-output/artifact-manifest.txt | xargs -r sha256sum > code-review-checklist-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > code-review-checklist-output/review-start-utc.txt
date > code-review-checklist-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > code-review-checklist-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' code-review-checklist-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 code-review-checklist-output/signal-hits.txt | sort | uniq -c | sort -nr > code-review-checklist-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > code-review-checklist-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > code-review-checklist-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' code-review-checklist-output/extension-summary.txt
```
...
git status --short > code-review-checklist-output/git-status.txt 2>/dev/null || true
git diff --stat > code-review-checklist-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > code-review-checklist-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > code-review-checklist-output/code-review-checklist-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > code-review-checklist-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' code-review-checklist-output/code-review-checklist-report-starter.md
```
...
find code-review-checklist-output -maxdepth 2 -type f | sort > code-review-checklist-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf code-review-checklist-output/code-review-checklist-bundle.tgz code-review-checklist-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf code-review-checklist-output/code-review-checklist-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 code-review-checklist-output/signal-hits.txt | sort | uniq -c | sort -nr > code-review-checklist-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' code-review-checklist-output/review-queue.txt > code-review-checklist-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' code-review-checklist-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > code-review-checklist-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> code-review-checklist-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' code-review-checklist-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > code-review-checklist-output/code-review-checklist-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > code-review-checklist-output/code-review-checklist-final.md
# Preview the output files for QA
sed -n '1,20p' code-review-checklist-output/code-review-checklist-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > code-review-checklist-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 code-review-checklist-output/code-review-checklist-bundle.tgz > code-review-checklist-output/code-review-checklist-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > code-review-checklist-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' code-review-checklist-output/review-queue.txt > code-review-checklist-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > code-review-checklist-output/permission-limited-files.txt
```
...
find code-review-checklist-output -maxdepth 2 -type f | sort > code-review-checklist-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > code-review-checklist-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' code-review-checklist-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > code-review-checklist-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > code-review-checklist-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > code-review-checklist-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > code-review-checklist-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > code-review-checklist-output/current-utc.txt
```
...
- produce: `code-review-checklist-output/code-review-checklist-report.md`
- produce: `code-review-checklist-output/output-manifest.txt`
- produce: `code-review-checklist-output/code-review-checklist-bundle.tgz`
- produce: `code-review-checklist-output/code-review-checklist-findings.csv`
...
- Confirm `code-review-checklist-output` contains the report, manifest, and at least one evidence artifact.
