...
Summary: Database design principles and decision-making. Schema design, indexing strategy, ORM selection, serverless databases.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `database-design` or the folder's specialized domain
...
```bash
mkdir -p database-design-output
pwd > database-design-output/pwd.txt
rg --files . | head -200 > database-design-output/repo-files.txt
git status --short > database-design-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper database-design review.
```bash
# Create the output directory for this skill
mkdir -p database-design-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > database-design-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > database-design-output/scope-note.txt
```
...
head -30 database-design-output/artifact-manifest.txt | xargs -r sha256sum > database-design-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > database-design-output/review-start-utc.txt
date > database-design-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > database-design-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' database-design-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 database-design-output/signal-hits.txt | sort | uniq -c | sort -nr > database-design-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > database-design-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > database-design-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' database-design-output/extension-summary.txt
```
...
git status --short > database-design-output/git-status.txt 2>/dev/null || true
git diff --stat > database-design-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > database-design-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > database-design-output/database-design-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > database-design-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' database-design-output/database-design-report-starter.md
```
...
find database-design-output -maxdepth 2 -type f | sort > database-design-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf database-design-output/database-design-bundle.tgz database-design-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf database-design-output/database-design-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 database-design-output/signal-hits.txt | sort | uniq -c | sort -nr > database-design-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' database-design-output/review-queue.txt > database-design-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' database-design-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > database-design-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> database-design-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' database-design-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > database-design-output/database-design-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > database-design-output/database-design-final.md
# Preview the output files for QA
sed -n '1,20p' database-design-output/database-design-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > database-design-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 database-design-output/database-design-bundle.tgz > database-design-output/database-design-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > database-design-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' database-design-output/review-queue.txt > database-design-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > database-design-output/permission-limited-files.txt
```
...
find database-design-output -maxdepth 2 -type f | sort > database-design-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > database-design-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' database-design-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > database-design-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > database-design-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > database-design-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > database-design-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > database-design-output/current-utc.txt
```
...
- produce: `database-design-output/database-design-report.md`
- produce: `database-design-output/output-manifest.txt`
- produce: `database-design-output/database-design-bundle.tgz`
- produce: `database-design-output/database-design-findings.csv`
...
- Confirm `database-design-output` contains the report, manifest, and at least one evidence artifact.
