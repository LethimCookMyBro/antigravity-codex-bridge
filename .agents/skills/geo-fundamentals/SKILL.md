...
- Use when the task clearly matches `geo-fundamentals` or the folder's specialized domain
...
```bash
mkdir -p geo-fundamentals-output
pwd > geo-fundamentals-output/pwd.txt
rg --files . | head -200 > geo-fundamentals-output/repo-files.txt
git status --short > geo-fundamentals-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper geo-fundamentals review.
```bash
# Create the output directory for this skill
mkdir -p geo-fundamentals-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > geo-fundamentals-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > geo-fundamentals-output/scope-note.txt
```
...
head -30 geo-fundamentals-output/artifact-manifest.txt | xargs -r sha256sum > geo-fundamentals-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > geo-fundamentals-output/review-start-utc.txt
date > geo-fundamentals-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > geo-fundamentals-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' geo-fundamentals-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 geo-fundamentals-output/signal-hits.txt | sort | uniq -c | sort -nr > geo-fundamentals-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > geo-fundamentals-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > geo-fundamentals-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' geo-fundamentals-output/extension-summary.txt
```
...
git status --short > geo-fundamentals-output/git-status.txt 2>/dev/null || true
git diff --stat > geo-fundamentals-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > geo-fundamentals-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > geo-fundamentals-output/geo-fundamentals-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > geo-fundamentals-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' geo-fundamentals-output/geo-fundamentals-report-starter.md
```
...
find geo-fundamentals-output -maxdepth 2 -type f | sort > geo-fundamentals-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf geo-fundamentals-output/geo-fundamentals-bundle.tgz geo-fundamentals-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf geo-fundamentals-output/geo-fundamentals-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 geo-fundamentals-output/signal-hits.txt | sort | uniq -c | sort -nr > geo-fundamentals-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' geo-fundamentals-output/review-queue.txt > geo-fundamentals-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' geo-fundamentals-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > geo-fundamentals-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> geo-fundamentals-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' geo-fundamentals-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > geo-fundamentals-output/geo-fundamentals-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > geo-fundamentals-output/geo-fundamentals-final.md
# Preview the output files for QA
sed -n '1,20p' geo-fundamentals-output/geo-fundamentals-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > geo-fundamentals-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 geo-fundamentals-output/geo-fundamentals-bundle.tgz > geo-fundamentals-output/geo-fundamentals-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > geo-fundamentals-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' geo-fundamentals-output/review-queue.txt > geo-fundamentals-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > geo-fundamentals-output/permission-limited-files.txt
```
...
find geo-fundamentals-output -maxdepth 2 -type f | sort > geo-fundamentals-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > geo-fundamentals-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' geo-fundamentals-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > geo-fundamentals-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > geo-fundamentals-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > geo-fundamentals-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > geo-fundamentals-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > geo-fundamentals-output/current-utc.txt
```
...
- produce: `geo-fundamentals-output/geo-fundamentals-report.md`
- produce: `geo-fundamentals-output/output-manifest.txt`
- produce: `geo-fundamentals-output/geo-fundamentals-bundle.tgz`
- produce: `geo-fundamentals-output/geo-fundamentals-findings.csv`
...
- Confirm `geo-fundamentals-output` contains the report, manifest, and at least one evidence artifact.
