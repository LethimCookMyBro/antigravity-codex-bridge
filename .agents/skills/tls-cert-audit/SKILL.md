---
skill: tls-cert-audit
name: tls-cert-audit
version: 1.0.0
source: codex/tls-cert-audit
...
python_min: 3.8
description: Inspect certificate metadata, negotiated TLS details, and weak-crypto acceptance with a local helper script for owned or authorized hosts.
allowed-tools: Read, Glob, Grep, Bash
---
# SKILL: TLS Certificate Audit

Summary: Inspect certificate metadata, negotiated TLS details, and weak-crypto acceptance with a local helper script for owned or authorized hosts.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.

## WHEN TO USE THIS SKILL

- Use when a user wants to inspect certificate details, issuer chains, SAN coverage, expiration windows, or weak TLS acceptance on an owned host
- Use when you need a lightweight certificate audit without a full external scanner
- Use when Codex should gather evidence with a local script and return a structured TLS hygiene summary
- Use when the target is clearly authorized and the checks stay low-noise
- Use when DNS ownership, web exposure, or crypto hygiene must be validated before deeper assessment

## KEY TECHNIQUES & TOOLS

### Phase 1: Confirm target and port

- Resolve the service endpoint first:
...
- Make sure the correct TLS port is in scope:
  - `443` for standard HTTPS
  - custom ports only when the user explicitly provides them

### Phase 2: Run the local TLS audit helper

- The repo includes `.agents/scripts/h4cker_tls_cert_audit.py`
- The script is adapted from the `h4cker` certificate info and weak-crypto examples
- Run it like this:
...
python3 .agents/scripts/h4cker_tls_cert_audit.py example.com --port 8443
...
- The script returns JSON with:
  ...
  - SAN entries
  - negotiated cipher
  - optional weak-cipher findings

```bash
mkdir -p tls-cert-audit-output
python3 .agents/scripts/h4cker_tls_cert_audit.py example.com --check-weak-ciphers > tls-cert-audit-output/tls-audit.json
jq '.certificate, .weak_cipher_findings' tls-cert-audit-output/tls-audit.json
```

### Phase 3: Interpret certificate hygiene

- Review:
  - whether SAN covers the hostname
  - whether issuer matches the expected CA
  - whether validity dates are close to expiry
  - whether protocol or cipher choices look outdated

- Treat weak-cipher acceptance as a review signal, not an automatic critical issue
- Compare the script result against what the service actually presents to this host and port

### Decision rules

- If the SAN list does not cover the requested hostname, treat hostname mismatch as a primary finding before discussing cipher strength
- If the service is behind a load balancer or CDN, compare the helper result against `openssl s_client` with explicit SNI before assuming the leaf certificate is wrong
- If weak-cipher acceptance appears only on a nonstandard port, report that per-port instead of generalizing it to the whole domain
- If the certificate is close to expiry but otherwise sound, classify it as operational risk rather than cryptographic weakness

### Phase 4: Cross-check with manual pivots

- Use complementary checks when needed:
...
```bash
mkdir -p tls-cert-audit-output
python3 .agents/scripts/h4cker_tls_cert_audit.py example.com > tls-cert-audit-output/tls-audit.json
jq '.certificate.subject_alt_names[]?.value' tls-cert-audit-output/tls-audit.json
```

- Use manual pivots to verify:
  - presented leaf certificate
  - CN and SAN coverage
  - expiry dates
  - negotiated protocol/cipher

- Prefer verifying one service endpoint at a time
...
- `Scope` must state:
  - hostname
  - port
  - whether weak-cipher probing was enabled

- `Certificate Summary` should include:
  - subject
  - issuer
  - SAN coverage
  - validity period

- `TLS Findings` must separate:
  - confirmed configuration details
  - weak or outdated signals
  - items requiring deeper manual review

- `Evidence` must include:
  - the script command used
  - any supporting `openssl` or `curl` commands
  - the most relevant JSON fields returned

- `Recommended Next Actions` should stay defensive:
  - certificate renewal planning
  - CA or SAN correction
  - protocol/cipher policy review
  - additional web or load-balancer validation
...
Use when you need a stable starting point before deeper tls-cert-audit review.
```bash
# Create the output directory for this skill
mkdir -p tls-cert-audit-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > tls-cert-audit-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > tls-cert-audit-output/scope-note.txt
```
...
head -30 tls-cert-audit-output/artifact-manifest.txt | xargs -r sha256sum > tls-cert-audit-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > tls-cert-audit-output/review-start-utc.txt
date > tls-cert-audit-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > tls-cert-audit-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' tls-cert-audit-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 tls-cert-audit-output/signal-hits.txt | sort | uniq -c | sort -nr > tls-cert-audit-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > tls-cert-audit-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > tls-cert-audit-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' tls-cert-audit-output/extension-summary.txt
```
...
git status --short > tls-cert-audit-output/git-status.txt 2>/dev/null || true
git diff --stat > tls-cert-audit-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > tls-cert-audit-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > tls-cert-audit-output/tls-cert-audit-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > tls-cert-audit-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' tls-cert-audit-output/tls-cert-audit-report-starter.md
```
...
find tls-cert-audit-output -maxdepth 2 -type f | sort > tls-cert-audit-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf tls-cert-audit-output/tls-cert-audit-bundle.tgz tls-cert-audit-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf tls-cert-audit-output/tls-cert-audit-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 tls-cert-audit-output/signal-hits.txt | sort | uniq -c | sort -nr > tls-cert-audit-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' tls-cert-audit-output/review-queue.txt > tls-cert-audit-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' tls-cert-audit-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > tls-cert-audit-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> tls-cert-audit-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' tls-cert-audit-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > tls-cert-audit-output/tls-cert-audit-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > tls-cert-audit-output/tls-cert-audit-final.md
# Preview the output files for QA
sed -n '1,20p' tls-cert-audit-output/tls-cert-audit-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > tls-cert-audit-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 tls-cert-audit-output/tls-cert-audit-bundle.tgz > tls-cert-audit-output/tls-cert-audit-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > tls-cert-audit-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' tls-cert-audit-output/review-queue.txt > tls-cert-audit-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > tls-cert-audit-output/permission-limited-files.txt
```
...
find tls-cert-audit-output -maxdepth 2 -type f | sort > tls-cert-audit-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > tls-cert-audit-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' tls-cert-audit-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > tls-cert-audit-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > tls-cert-audit-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > tls-cert-audit-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > tls-cert-audit-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > tls-cert-audit-output/current-utc.txt
```
...
- produce: `tls-cert-audit-output/tls-cert-audit-report.md`
- produce: `tls-cert-audit-output/output-manifest.txt`
- produce: `tls-cert-audit-output/tls-cert-audit-bundle.tgz`
- produce: `tls-cert-audit-output/tls-cert-audit-findings.csv`
...
- Confirm `tls-cert-audit-output` contains the report, manifest, and at least one evidence artifact.
