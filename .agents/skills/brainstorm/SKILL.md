...
Summary: Structured brainstorming for projects and features. Explores multiple options before implementation.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `brainstorm` or the folder's specialized domain
...
```bash
mkdir -p brainstorm-output
pwd > brainstorm-output/pwd.txt
rg --files . | head -200 > brainstorm-output/repo-files.txt
git status --short > brainstorm-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper brainstorm review.
```bash
# Create the output directory for this skill
mkdir -p brainstorm-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > brainstorm-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > brainstorm-output/scope-note.txt
```
...
head -30 brainstorm-output/artifact-manifest.txt | xargs -r sha256sum > brainstorm-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > brainstorm-output/review-start-utc.txt
date > brainstorm-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > brainstorm-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' brainstorm-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 brainstorm-output/signal-hits.txt | sort | uniq -c | sort -nr > brainstorm-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > brainstorm-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > brainstorm-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' brainstorm-output/extension-summary.txt
```
...
git status --short > brainstorm-output/git-status.txt 2>/dev/null || true
git diff --stat > brainstorm-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > brainstorm-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > brainstorm-output/brainstorm-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > brainstorm-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' brainstorm-output/brainstorm-report-starter.md
```
...
find brainstorm-output -maxdepth 2 -type f | sort > brainstorm-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf brainstorm-output/brainstorm-bundle.tgz brainstorm-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf brainstorm-output/brainstorm-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 brainstorm-output/signal-hits.txt | sort | uniq -c | sort -nr > brainstorm-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' brainstorm-output/review-queue.txt > brainstorm-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' brainstorm-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > brainstorm-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> brainstorm-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' brainstorm-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > brainstorm-output/brainstorm-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > brainstorm-output/brainstorm-final.md
# Preview the output files for QA
sed -n '1,20p' brainstorm-output/brainstorm-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > brainstorm-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 brainstorm-output/brainstorm-bundle.tgz > brainstorm-output/brainstorm-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > brainstorm-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' brainstorm-output/review-queue.txt > brainstorm-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > brainstorm-output/permission-limited-files.txt
```
...
find brainstorm-output -maxdepth 2 -type f | sort > brainstorm-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > brainstorm-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' brainstorm-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > brainstorm-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > brainstorm-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > brainstorm-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > brainstorm-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > brainstorm-output/current-utc.txt
```
...
- produce: `brainstorm-output/brainstorm-report.md`
- produce: `brainstorm-output/output-manifest.txt`
- produce: `brainstorm-output/brainstorm-bundle.tgz`
- produce: `brainstorm-output/brainstorm-findings.csv`
...
- Confirm `brainstorm-output` contains the report, manifest, and at least one evidence artifact.
