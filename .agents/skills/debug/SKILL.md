...
Summary: Debugging command. Activates DEBUG mode for systematic problem investigation.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `debug` or the folder's specialized domain
...
This skill adapts the Antigravity `/debug` workflow for Codex.
Treat the user's message as the issue to investigate.

## Purpose

This skill activates DEBUG mode for systematic investigation of issues,
...
- If the issue is not reproducible, ask for the exact command, input, or file needed to reproduce it
- If no logs or error text exist, create the smallest reliable reproducer before changing code
- If multiple plausible causes remain, rank them and test the least destructive hypothesis first
- If the fix cannot be verified locally, say exactly what was changed and what still needs confirmation
...
- Ask before assuming. Get the full error context.
- Test hypotheses instead of guessing.
- Explain why, not just what changed.
- Prevent recurrence with tests, validation, or safeguards.
...
pwd > debug-output/pwd.txt
rg --files . | head -200 > debug-output/repo-files.txt
git status --short > debug-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper debug review.
```bash
# Create the output directory for this skill
mkdir -p debug-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > debug-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > debug-output/scope-note.txt
```
...
head -30 debug-output/artifact-manifest.txt | xargs -r sha256sum > debug-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > debug-output/review-start-utc.txt
date > debug-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > debug-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' debug-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 debug-output/signal-hits.txt | sort | uniq -c | sort -nr > debug-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > debug-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > debug-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' debug-output/extension-summary.txt
```
...
git status --short > debug-output/git-status.txt 2>/dev/null || true
git diff --stat > debug-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > debug-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > debug-output/debug-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > debug-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' debug-output/debug-report-starter.md
```
...
find debug-output -maxdepth 2 -type f | sort > debug-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf debug-output/debug-bundle.tgz debug-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf debug-output/debug-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 debug-output/signal-hits.txt | sort | uniq -c | sort -nr > debug-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' debug-output/review-queue.txt > debug-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' debug-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > debug-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> debug-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' debug-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > debug-output/debug-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > debug-output/debug-final.md
# Preview the output files for QA
sed -n '1,20p' debug-output/debug-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > debug-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 debug-output/debug-bundle.tgz > debug-output/debug-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > debug-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' debug-output/review-queue.txt > debug-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > debug-output/permission-limited-files.txt
```
...
find debug-output -maxdepth 2 -type f | sort > debug-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > debug-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' debug-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > debug-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > debug-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > debug-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > debug-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > debug-output/current-utc.txt
```
...
- produce: `debug-output/debug-report.md`
- produce: `debug-output/output-manifest.txt`
- produce: `debug-output/debug-bundle.tgz`
- produce: `debug-output/debug-findings.csv`
...
- Confirm `debug-output` contains the report, manifest, and at least one evidence artifact.
