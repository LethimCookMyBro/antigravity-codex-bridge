...
Summary: Test generation and test running command. Creates and executes tests for code.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `test` or the folder's specialized domain
...
- If no test framework exists, propose the smallest framework addition instead of pretending tests were run
- If tests depend on missing services, stub or mock them and state the assumption
- If execution is too expensive or blocked, produce the test file plus the exact run command that still needs confirmation
...
Use repo helpers when available:
...
```bash
mkdir -p test-output
pwd > test-output/pwd.txt
rg --files . | head -200 > test-output/repo-files.txt
git status --short > test-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper test review.
```bash
# Create the output directory for this skill
mkdir -p test-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > test-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > test-output/scope-note.txt
```
...
head -30 test-output/artifact-manifest.txt | xargs -r sha256sum > test-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > test-output/review-start-utc.txt
date > test-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > test-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' test-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 test-output/signal-hits.txt | sort | uniq -c | sort -nr > test-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > test-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > test-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' test-output/extension-summary.txt
```
...
git status --short > test-output/git-status.txt 2>/dev/null || true
git diff --stat > test-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > test-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > test-output/test-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > test-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' test-output/test-report-starter.md
```
...
find test-output -maxdepth 2 -type f | sort > test-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf test-output/test-bundle.tgz test-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf test-output/test-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 test-output/signal-hits.txt | sort | uniq -c | sort -nr > test-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' test-output/review-queue.txt > test-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' test-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > test-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> test-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' test-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > test-output/test-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > test-output/test-final.md
# Preview the output files for QA
sed -n '1,20p' test-output/test-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > test-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 test-output/test-bundle.tgz > test-output/test-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > test-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' test-output/review-queue.txt > test-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > test-output/permission-limited-files.txt
```
...
find test-output -maxdepth 2 -type f | sort > test-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > test-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' test-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > test-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > test-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > test-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > test-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > test-output/current-utc.txt
```
...
- produce: `test-output/test-report.md`
- produce: `test-output/output-manifest.txt`
- produce: `test-output/test-bundle.tgz`
- produce: `test-output/test-findings.csv`
...
- Confirm `test-output` contains the report, manifest, and at least one evidence artifact.
