---
skill: h4cker-skill-architect
name: h4cker-skill-architect
version: 1.0.0
source: codex/h4cker-skill-architect
...
description: Distill dense h4cker folders into compact, high-signal Codex skills without pushing the raw source.
allowed-tools: Read, Glob, Grep, Bash
---
# SKILL: H4cker Skill Architect

Summary: Distill dense h4cker folders into compact, high-signal Codex skills without pushing the raw source.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.

## WHEN TO USE THIS SKILL

- Use when a folder inside local `h4cker/` should become a publishable skill under `.agents/skills/`
- Use when the source is dense, repetitive, or mixed across README files, notes, tool lists, and scripts
- Use when the raw source should stay local-only while the distilled skill must be committed and pushed
- Use when one topic needs a Codex field manual rather than a pasted bookmark collection
- Use when a source folder is good but still needs normalization, safety framing, and a sharper output contract

## KEY TECHNIQUES & TOOLS


### Phase 0: Capture the current scope and workspace context

```bash
mkdir -p h4cker-skill-architect-output
pwd > h4cker-skill-architect-output/pwd.txt
rg --files . | head -200 > h4cker-skill-architect-output/repo-files.txt
git status --short > h4cker-skill-architect-output/git-status.txt 2>/dev/null || true
```

### Phase 1: Triage the source

- Read the folder `README.md` first
- Read only the 2 to 8 most operationally useful supporting files
- Prefer files that contain:
  ...
  - tool families
  - lab exercises
  - scripts with obvious CLI entry points
- Deprioritize:
  - giant URL dumps
  - screenshots
  - badges
...
- Infer the folder's primary job before writing
- Choose one of these skill shapes:
  - passive recon
  - active validation
  - hardening
  - detection and triage
  - malware or reverse-engineering analysis
  - cloud or identity audit
  - incident response workflow
- If the folder is too broad, extract one core theme instead of making a vague mega-skill
- If the folder naturally splits, create a focused skill now and leave the rest for later

### Phase 3: Extract the high-signal content
...
- Preserve:
  ...
  - reporting structure
- Convert large tool lists into categories such as:
  - discovery
  - enrichment
  - validation
  - visualization
  - reporting
- Keep real commands when they are safe, authorized, and actually useful
- Summarize scripts by purpose and invocation pattern, not by copying code

### Phase 4: Compress without losing value

- Merge duplicates aggressively
- Normalize synonyms and repeated tool references
- Group by phase, not by source filename
- Retain the commands that anchor the workflow
- Keep enough detail that Codex can execute without reopening the original folder
- Do not reduce everything to vague bullets if the source already gives strong commands

### Phase 5: Apply safety framing

- Reframe offensive material for authorized assessment, lab validation, or defensive review
- Remove or soften:
  ...
  - exploit chains
- Preserve safer patterns such as:
  - public-source enumeration
  - inventory
...
- Write the result to the target generated skill path, for example `skills/<slug>/SKILL.md`
- Add `agents/openai.yaml` when the topic needs a better display name and summary
- Keep the final skill under 220 lines
- Keep exactly 3 body sections:
  - `## WHEN TO USE THIS SKILL`
  - `## KEY TECHNIQUES & TOOLS`
  - `## OUTPUT FORMAT`
...
- Start with the strongest command or workflow phase first
- Put tool clusters inside subsections under `KEY TECHNIQUES & TOOLS`
- End with a strict report schema that forces evidence-backed output

### Review checklist

- Does the skill preserve the best commands from the source?
- Does it preserve the actual workflow order?
- Is it denser than a generic summary?
- Could a user delete the local source clone and still keep the useful behavior?

## OUTPUT FORMAT

- Start with Codex frontmatter:
...
- Then produce only:
  - trigger conditions
  - grouped techniques with real command examples
  - a strict final answer schema for Codex
- The finished skill must be:
  - compact
  - operational
  - source-grounded
  - safe to publish without the original `h4cker/` folder
...
Use when you need a stable starting point before deeper h4cker-skill-architect review.
```bash
# Create the output directory for this skill
mkdir -p h4cker-skill-architect-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > h4cker-skill-architect-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > h4cker-skill-architect-output/scope-note.txt
```
...
head -30 h4cker-skill-architect-output/artifact-manifest.txt | xargs -r sha256sum > h4cker-skill-architect-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > h4cker-skill-architect-output/review-start-utc.txt
date > h4cker-skill-architect-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > h4cker-skill-architect-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' h4cker-skill-architect-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 h4cker-skill-architect-output/signal-hits.txt | sort | uniq -c | sort -nr > h4cker-skill-architect-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > h4cker-skill-architect-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > h4cker-skill-architect-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' h4cker-skill-architect-output/extension-summary.txt
```
...
git status --short > h4cker-skill-architect-output/git-status.txt 2>/dev/null || true
git diff --stat > h4cker-skill-architect-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > h4cker-skill-architect-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > h4cker-skill-architect-output/h4cker-skill-architect-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > h4cker-skill-architect-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' h4cker-skill-architect-output/h4cker-skill-architect-report-starter.md
```
...
find h4cker-skill-architect-output -maxdepth 2 -type f | sort > h4cker-skill-architect-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf h4cker-skill-architect-output/h4cker-skill-architect-bundle.tgz h4cker-skill-architect-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf h4cker-skill-architect-output/h4cker-skill-architect-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 h4cker-skill-architect-output/signal-hits.txt | sort | uniq -c | sort -nr > h4cker-skill-architect-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' h4cker-skill-architect-output/review-queue.txt > h4cker-skill-architect-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' h4cker-skill-architect-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > h4cker-skill-architect-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> h4cker-skill-architect-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' h4cker-skill-architect-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > h4cker-skill-architect-output/h4cker-skill-architect-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > h4cker-skill-architect-output/h4cker-skill-architect-final.md
# Preview the output files for QA
sed -n '1,20p' h4cker-skill-architect-output/h4cker-skill-architect-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > h4cker-skill-architect-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 h4cker-skill-architect-output/h4cker-skill-architect-bundle.tgz > h4cker-skill-architect-output/h4cker-skill-architect-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > h4cker-skill-architect-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' h4cker-skill-architect-output/review-queue.txt > h4cker-skill-architect-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > h4cker-skill-architect-output/permission-limited-files.txt
```
...
find h4cker-skill-architect-output -maxdepth 2 -type f | sort > h4cker-skill-architect-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > h4cker-skill-architect-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' h4cker-skill-architect-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > h4cker-skill-architect-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > h4cker-skill-architect-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > h4cker-skill-architect-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > h4cker-skill-architect-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > h4cker-skill-architect-output/current-utc.txt
```
...
- produce: `h4cker-skill-architect-output/h4cker-skill-architect-report.md`
- produce: `h4cker-skill-architect-output/output-manifest.txt`
- produce: `h4cker-skill-architect-output/h4cker-skill-architect-bundle.tgz`
- produce: `h4cker-skill-architect-output/h4cker-skill-architect-findings.csv`
...
- Confirm `h4cker-skill-architect-output` contains the report, manifest, and at least one evidence artifact.
