...
Summary: Create project plan using project-planner agent. No code writing, only plan file generation.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `plan` or the folder's specialized domain
...
- If the request is too vague for a good plan, stop and ask the minimum clarifying questions first
- If the repo already contains planning docs, create a new plan only when the task is distinct; otherwise update the existing plan
- If a precise slug cannot be derived safely, use a short generic slug and state the assumption
...
```bash
mkdir -p plan-output
pwd > plan-output/pwd.txt
rg --files . | head -200 > plan-output/repo-files.txt
git status --short > plan-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper plan review.
```bash
# Create the output directory for this skill
mkdir -p plan-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > plan-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > plan-output/scope-note.txt
```
...
head -30 plan-output/artifact-manifest.txt | xargs -r sha256sum > plan-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > plan-output/review-start-utc.txt
date > plan-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > plan-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' plan-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 plan-output/signal-hits.txt | sort | uniq -c | sort -nr > plan-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > plan-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > plan-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' plan-output/extension-summary.txt
```
...
git status --short > plan-output/git-status.txt 2>/dev/null || true
git diff --stat > plan-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > plan-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > plan-output/plan-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > plan-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' plan-output/plan-report-starter.md
```
...
find plan-output -maxdepth 2 -type f | sort > plan-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf plan-output/plan-bundle.tgz plan-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf plan-output/plan-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 plan-output/signal-hits.txt | sort | uniq -c | sort -nr > plan-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' plan-output/review-queue.txt > plan-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' plan-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > plan-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> plan-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' plan-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > plan-output/plan-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > plan-output/plan-final.md
# Preview the output files for QA
sed -n '1,20p' plan-output/plan-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > plan-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 plan-output/plan-bundle.tgz > plan-output/plan-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > plan-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' plan-output/review-queue.txt > plan-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > plan-output/permission-limited-files.txt
```
...
find plan-output -maxdepth 2 -type f | sort > plan-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > plan-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' plan-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > plan-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > plan-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > plan-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > plan-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > plan-output/current-utc.txt
```
...
- produce: `plan-output/plan-report.md`
- produce: `plan-output/output-manifest.txt`
- produce: `plan-output/plan-bundle.tgz`
- produce: `plan-output/plan-findings.csv`
...
- Confirm `plan-output` contains the report, manifest, and at least one evidence artifact.
