---
skill: python-ruby-and-bash
name: python-ruby-and-bash
version: 1.0.0
source: h4cker/python-ruby-and-bash
...
python_min: 3.8
description: Choose simple scripting patterns for defensive automation, parsing, and reproducible system checks after the main security objective is already scoped.
allowed-tools: Read, Glob, Grep, Bash
---
# SKILL: Python Ruby and Bash

Summary: Choose simple scripting patterns for defensive automation, parsing, and reproducible system checks.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.

## WHEN TO USE THIS SKILL

- Use when the task clearly maps to python-ruby-and-bash topics in an owned, authorized, or lab context
...
```bash
mkdir -p python-ruby-and-bash-output
find . -maxdepth 3 -type f | sort | head -100 > python-ruby-and-bash-output/files.txt
ls -la python-ruby-and-bash-output
```

### Phase 2: Review high-signal artifacts

```bash
rg -n "error|alert|warning|TODO|FIXME|password|token|secret" . 2>/dev/null | head -100 > python-ruby-and-bash-output/signals.txt || true
sed -n '1,40p' python-ruby-and-bash-output/signals.txt
```

### Phase 3: Preserve reproducible evidence

```bash
find . -maxdepth 4 -type f | sort > python-ruby-and-bash-output/inventory.txt
wc -l python-ruby-and-bash-output/inventory.txt python-ruby-and-bash-output/signals.txt 2>/dev/null || true
```
...
- `Artifacts` should list:
  - `python-ruby-and-bash-output/files.txt`
  - `python-ruby-and-bash-output/signals.txt`
  - `python-ruby-and-bash-output/inventory.txt`
...
Use when you need a stable starting point before deeper python-ruby-and-bash review.
```bash
# Create the output directory for this skill
mkdir -p python-ruby-and-bash-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > python-ruby-and-bash-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > python-ruby-and-bash-output/scope-note.txt
```
...
head -30 python-ruby-and-bash-output/artifact-manifest.txt | xargs -r sha256sum > python-ruby-and-bash-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > python-ruby-and-bash-output/review-start-utc.txt
date > python-ruby-and-bash-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > python-ruby-and-bash-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' python-ruby-and-bash-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 python-ruby-and-bash-output/signal-hits.txt | sort | uniq -c | sort -nr > python-ruby-and-bash-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > python-ruby-and-bash-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > python-ruby-and-bash-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' python-ruby-and-bash-output/extension-summary.txt
```
...
git status --short > python-ruby-and-bash-output/git-status.txt 2>/dev/null || true
git diff --stat > python-ruby-and-bash-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > python-ruby-and-bash-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > python-ruby-and-bash-output/python-ruby-and-bash-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > python-ruby-and-bash-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' python-ruby-and-bash-output/python-ruby-and-bash-report-starter.md
```
...
find python-ruby-and-bash-output -maxdepth 2 -type f | sort > python-ruby-and-bash-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf python-ruby-and-bash-output/python-ruby-and-bash-bundle.tgz python-ruby-and-bash-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf python-ruby-and-bash-output/python-ruby-and-bash-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 python-ruby-and-bash-output/signal-hits.txt | sort | uniq -c | sort -nr > python-ruby-and-bash-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' python-ruby-and-bash-output/review-queue.txt > python-ruby-and-bash-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' python-ruby-and-bash-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > python-ruby-and-bash-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> python-ruby-and-bash-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' python-ruby-and-bash-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > python-ruby-and-bash-output/python-ruby-and-bash-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > python-ruby-and-bash-output/python-ruby-and-bash-final.md
# Preview the output files for QA
sed -n '1,20p' python-ruby-and-bash-output/python-ruby-and-bash-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > python-ruby-and-bash-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 python-ruby-and-bash-output/python-ruby-and-bash-bundle.tgz > python-ruby-and-bash-output/python-ruby-and-bash-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > python-ruby-and-bash-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' python-ruby-and-bash-output/review-queue.txt > python-ruby-and-bash-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > python-ruby-and-bash-output/permission-limited-files.txt
```
...
find python-ruby-and-bash-output -maxdepth 2 -type f | sort > python-ruby-and-bash-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > python-ruby-and-bash-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' python-ruby-and-bash-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > python-ruby-and-bash-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > python-ruby-and-bash-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > python-ruby-and-bash-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > python-ruby-and-bash-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > python-ruby-and-bash-output/current-utc.txt
```
...
- produce: `python-ruby-and-bash-output/python-ruby-and-bash-report.md`
- produce: `python-ruby-and-bash-output/output-manifest.txt`
- produce: `python-ruby-and-bash-output/python-ruby-and-bash-bundle.tgz`
- produce: `python-ruby-and-bash-output/python-ruby-and-bash-findings.csv`
...
- Confirm `python-ruby-and-bash-output` contains the report, manifest, and at least one evidence artifact.
