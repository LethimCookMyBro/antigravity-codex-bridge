...
version: 1.0.0
source: codex/tailwind-patterns
...
Summary: Tailwind CSS v4 principles. CSS-first configuration, container queries, modern patterns, design token architecture.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `tailwind-patterns` or the folder's specialized domain
...
```bash
mkdir -p tailwind-patterns-output
pwd > tailwind-patterns-output/pwd.txt
rg --files . | head -200 > tailwind-patterns-output/repo-files.txt
git status --short > tailwind-patterns-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper tailwind-patterns review.
```bash
# Create the output directory for this skill
mkdir -p tailwind-patterns-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > tailwind-patterns-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > tailwind-patterns-output/scope-note.txt
```
...
head -30 tailwind-patterns-output/artifact-manifest.txt | xargs -r sha256sum > tailwind-patterns-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > tailwind-patterns-output/review-start-utc.txt
date > tailwind-patterns-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > tailwind-patterns-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' tailwind-patterns-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 tailwind-patterns-output/signal-hits.txt | sort | uniq -c | sort -nr > tailwind-patterns-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > tailwind-patterns-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > tailwind-patterns-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' tailwind-patterns-output/extension-summary.txt
```
...
git status --short > tailwind-patterns-output/git-status.txt 2>/dev/null || true
git diff --stat > tailwind-patterns-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > tailwind-patterns-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > tailwind-patterns-output/tailwind-patterns-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > tailwind-patterns-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' tailwind-patterns-output/tailwind-patterns-report-starter.md
```
...
find tailwind-patterns-output -maxdepth 2 -type f | sort > tailwind-patterns-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf tailwind-patterns-output/tailwind-patterns-bundle.tgz tailwind-patterns-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf tailwind-patterns-output/tailwind-patterns-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 tailwind-patterns-output/signal-hits.txt | sort | uniq -c | sort -nr > tailwind-patterns-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' tailwind-patterns-output/review-queue.txt > tailwind-patterns-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' tailwind-patterns-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > tailwind-patterns-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> tailwind-patterns-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' tailwind-patterns-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > tailwind-patterns-output/tailwind-patterns-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > tailwind-patterns-output/tailwind-patterns-final.md
# Preview the output files for QA
sed -n '1,20p' tailwind-patterns-output/tailwind-patterns-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > tailwind-patterns-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 tailwind-patterns-output/tailwind-patterns-bundle.tgz > tailwind-patterns-output/tailwind-patterns-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > tailwind-patterns-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' tailwind-patterns-output/review-queue.txt > tailwind-patterns-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > tailwind-patterns-output/permission-limited-files.txt
```
...
find tailwind-patterns-output -maxdepth 2 -type f | sort > tailwind-patterns-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > tailwind-patterns-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' tailwind-patterns-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > tailwind-patterns-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > tailwind-patterns-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > tailwind-patterns-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > tailwind-patterns-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > tailwind-patterns-output/current-utc.txt
```
...
- produce: `tailwind-patterns-output/tailwind-patterns-report.md`
- produce: `tailwind-patterns-output/output-manifest.txt`
- produce: `tailwind-patterns-output/tailwind-patterns-bundle.tgz`
- produce: `tailwind-patterns-output/tailwind-patterns-findings.csv`
...
- Confirm `tailwind-patterns-output` contains the report, manifest, and at least one evidence artifact.
