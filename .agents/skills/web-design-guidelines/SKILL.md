...
version: 1.0.0
source: codex/web-design-guidelines
...
- Use when the task clearly matches `web-design-guidelines` or the folder's specialized domain
...
```bash
mkdir -p web-design-guidelines-output
pwd > web-design-guidelines-output/pwd.txt
rg --files . | head -200 > web-design-guidelines-output/repo-files.txt
git status --short > web-design-guidelines-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper web-design-guidelines review.
```bash
# Create the output directory for this skill
mkdir -p web-design-guidelines-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > web-design-guidelines-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > web-design-guidelines-output/scope-note.txt
```
...
head -30 web-design-guidelines-output/artifact-manifest.txt | xargs -r sha256sum > web-design-guidelines-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > web-design-guidelines-output/review-start-utc.txt
date > web-design-guidelines-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > web-design-guidelines-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' web-design-guidelines-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 web-design-guidelines-output/signal-hits.txt | sort | uniq -c | sort -nr > web-design-guidelines-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > web-design-guidelines-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > web-design-guidelines-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' web-design-guidelines-output/extension-summary.txt
```
...
git status --short > web-design-guidelines-output/git-status.txt 2>/dev/null || true
git diff --stat > web-design-guidelines-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > web-design-guidelines-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > web-design-guidelines-output/web-design-guidelines-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > web-design-guidelines-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' web-design-guidelines-output/web-design-guidelines-report-starter.md
```
...
find web-design-guidelines-output -maxdepth 2 -type f | sort > web-design-guidelines-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf web-design-guidelines-output/web-design-guidelines-bundle.tgz web-design-guidelines-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf web-design-guidelines-output/web-design-guidelines-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 web-design-guidelines-output/signal-hits.txt | sort | uniq -c | sort -nr > web-design-guidelines-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' web-design-guidelines-output/review-queue.txt > web-design-guidelines-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' web-design-guidelines-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > web-design-guidelines-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> web-design-guidelines-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' web-design-guidelines-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > web-design-guidelines-output/web-design-guidelines-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > web-design-guidelines-output/web-design-guidelines-final.md
# Preview the output files for QA
sed -n '1,20p' web-design-guidelines-output/web-design-guidelines-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > web-design-guidelines-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 web-design-guidelines-output/web-design-guidelines-bundle.tgz > web-design-guidelines-output/web-design-guidelines-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > web-design-guidelines-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' web-design-guidelines-output/review-queue.txt > web-design-guidelines-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > web-design-guidelines-output/permission-limited-files.txt
```
...
find web-design-guidelines-output -maxdepth 2 -type f | sort > web-design-guidelines-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > web-design-guidelines-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' web-design-guidelines-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > web-design-guidelines-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > web-design-guidelines-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > web-design-guidelines-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > web-design-guidelines-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > web-design-guidelines-output/current-utc.txt
```
...
- produce: `web-design-guidelines-output/web-design-guidelines-report.md`
- produce: `web-design-guidelines-output/output-manifest.txt`
- produce: `web-design-guidelines-output/web-design-guidelines-bundle.tgz`
- produce: `web-design-guidelines-output/web-design-guidelines-findings.csv`
...
- Confirm `web-design-guidelines-output` contains the report, manifest, and at least one evidence artifact.
