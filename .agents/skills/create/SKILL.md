...
Summary: Create new application command. Triggers App Builder skill and starts interactive dialogue with user.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `create` or the folder's specialized domain
...
- whether this is a brand-new project or an existing repo
- the primary user and core workflow
- the minimum feature set needed for the first iteration
- the stack constraints already implied by the repo or the request
...
- If the repo already exists and the request is incremental, switch to `$enhance` instead of scaffolding a second app
- If the user request is too broad, reduce the first milestone to one end-to-end flow and state the deferred work
- If the stack is unspecified, prefer the stack already present in the repository; otherwise propose one default stack and explain why
- If preview tooling cannot start, report the exact blocking command or missing dependency instead of claiming the app is ready
...
The `How to Run` section must include the real local command if one was created or detected.
...
```bash
mkdir -p create-output
pwd > create-output/pwd.txt
rg --files . | head -200 > create-output/repo-files.txt
git status --short > create-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper create review.
```bash
# Create the output directory for this skill
mkdir -p create-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > create-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > create-output/scope-note.txt
```
...
head -30 create-output/artifact-manifest.txt | xargs -r sha256sum > create-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > create-output/review-start-utc.txt
date > create-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > create-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' create-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 create-output/signal-hits.txt | sort | uniq -c | sort -nr > create-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > create-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > create-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' create-output/extension-summary.txt
```
...
git status --short > create-output/git-status.txt 2>/dev/null || true
git diff --stat > create-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > create-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > create-output/create-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > create-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' create-output/create-report-starter.md
```
...
find create-output -maxdepth 2 -type f | sort > create-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf create-output/create-bundle.tgz create-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf create-output/create-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 create-output/signal-hits.txt | sort | uniq -c | sort -nr > create-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' create-output/review-queue.txt > create-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' create-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > create-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> create-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' create-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > create-output/create-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > create-output/create-final.md
# Preview the output files for QA
sed -n '1,20p' create-output/create-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > create-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 create-output/create-bundle.tgz > create-output/create-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > create-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' create-output/review-queue.txt > create-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > create-output/permission-limited-files.txt
```
...
find create-output -maxdepth 2 -type f | sort > create-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > create-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' create-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > create-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > create-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > create-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > create-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > create-output/current-utc.txt
```
...
- produce: `create-output/create-report.md`
- produce: `create-output/output-manifest.txt`
- produce: `create-output/create-bundle.tgz`
- produce: `create-output/create-findings.csv`
...
- Confirm `create-output` contains the report, manifest, and at least one evidence artifact.
