...
python_min: 3.8
description: Defensive threat-hunting workflow for hypotheses, pivots, evidence correlation, and artifact-ready outputs.
allowed-tools: Read, Glob, Grep, Bash
---
# SKILL: Threat Hunting

Summary: Defensive threat-hunting workflow for hypotheses, pivots, evidence correlation, and artifact-ready outputs.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.

## WHEN TO USE THIS SKILL

- Use when the task is to confirm or refute suspicious activity across logs and telemetry
- Use when IOC lists, auth anomalies, or exposure findings need broader hunting
- Use when the user needs hunt hypotheses and evidence-backed pivots

## INPUT FROM

- `dfir` indicators
- `osint-recon` exposed asset findings
- auth, network, proxy, or EDR logs

## KEY TECHNIQUES & TOOLS

### Phase 1: Define the hunt hypothesis, time window, and log sources

```bash
mkdir -p threat-hunting-output
printf '%s\n' "hypothesis=suspicious admin access outside baseline" "window=last 24h" "sources=auth.log,proxy.log,dns.log,edr.csv" > threat-hunting-output/hunt-scope.txt
cat threat-hunting-output/hunt-scope.txt
```

### Phase 2: Hunt and pivot across at least two telemetry sources

```bash
rg -n "203.0.113.45|alice|sudo:|wget|curl" /path/to/logs > threat-hunting-output/raw-hits.txt
python3 .agents/scripts/h4cker_ai_log_analysis.py /path/to/logs/security.log --output threat-hunting-output/hunt-analysis.json
sed -n '1,80p' threat-hunting-output/raw-hits.txt
```

### Phase 3: Promote or dismiss the hypothesis with explicit confidence

```bash
printf "indicator\tsource\tconfidence\tnote\n" > threat-hunting-output/hunt-summary.tsv
wc -l threat-hunting-output/raw-hits.txt threat-hunting-output/hunt-analysis.json 2>/dev/null || true
```

### Decision rules

- If the only support is a single weak indicator, keep the result as suspicious, not confirmed
- If the hunt produces cross-source correlation, promote the result into `dfir`
- If the environment lacks enough telemetry, say what additional sources are needed
- If the same pattern matches expected admin maintenance, downgrade the finding and record the baseline assumption
- If one source contradicts another, keep both artifacts and mark the hypothesis unresolved instead of forcing a conclusion
...
- `Artifacts` should list:
  - `threat-hunting-output/hunt-scope.txt`
  - `threat-hunting-output/raw-hits.txt`
  - `threat-hunting-output/hunt-analysis.json`
  - `threat-hunting-output/hunt-summary.tsv`

## Quick Mode (< 5 minutes)

- Define one hypothesis and one time window only.
- Hunt across one primary log source and one confirming source.
- Stop once you can say `supported`, `unsupported`, or `needs more data`.
...
- If the log volume is too large, narrow by one host, user, or 15-minute window before widening again.
- If the timestamps disagree across sources, normalize timezone first and note the assumption.
- Edge case 1: the environment is IPv6-heavy; include IPv6 forms in all hunt expressions.
- Edge case 2: the signal appears only in cloud control-plane logs; state that host telemetry alone is insufficient.


## Phase Output Map

- Phase 1 output: written hypothesis, time window, and source list.
- Phase 2 output: raw hits plus parsed correlation evidence.
- Phase 3 output: explicit confidence and next-collection decision.


## Done When

- Scope is fixed to one hypothesis and time window.
- Evidence exists from at least one primary source and one supporting or contradicting source.
- The hunt ends with `supported`, `unsupported`, or `needs more data`, not a vague note.

## Technique Depth

### Technique 1: Artifact manifest and scope note
Use when you need a stable starting point before deeper threat-hunting review.
```bash
# Create the output directory for this skill
mkdir -p threat-hunting-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > threat-hunting-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > threat-hunting-output/scope-note.txt
```
...
head -30 threat-hunting-output/artifact-manifest.txt | xargs -r sha256sum > threat-hunting-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > threat-hunting-output/review-start-utc.txt
date > threat-hunting-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > threat-hunting-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' threat-hunting-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 threat-hunting-output/signal-hits.txt | sort | uniq -c | sort -nr > threat-hunting-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > threat-hunting-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > threat-hunting-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' threat-hunting-output/extension-summary.txt
```
...
git status --short > threat-hunting-output/git-status.txt 2>/dev/null || true
git diff --stat > threat-hunting-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > threat-hunting-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > threat-hunting-output/threat-hunting-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > threat-hunting-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' threat-hunting-output/threat-hunting-report-starter.md
```
...
find threat-hunting-output -maxdepth 2 -type f | sort > threat-hunting-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf threat-hunting-output/threat-hunting-bundle.tgz threat-hunting-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf threat-hunting-output/threat-hunting-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 threat-hunting-output/signal-hits.txt | sort | uniq -c | sort -nr > threat-hunting-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' threat-hunting-output/review-queue.txt > threat-hunting-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' threat-hunting-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > threat-hunting-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> threat-hunting-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' threat-hunting-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > threat-hunting-output/threat-hunting-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > threat-hunting-output/threat-hunting-final.md
# Preview the output files for QA
sed -n '1,20p' threat-hunting-output/threat-hunting-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > threat-hunting-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 threat-hunting-output/threat-hunting-bundle.tgz > threat-hunting-output/threat-hunting-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > threat-hunting-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' threat-hunting-output/review-queue.txt > threat-hunting-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > threat-hunting-output/permission-limited-files.txt
```
...
find threat-hunting-output -maxdepth 2 -type f | sort > threat-hunting-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > threat-hunting-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' threat-hunting-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > threat-hunting-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > threat-hunting-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > threat-hunting-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > threat-hunting-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > threat-hunting-output/current-utc.txt
```
...
- produce: `threat-hunting-output/threat-hunting-report.md`
- produce: `threat-hunting-output/output-manifest.txt`
- produce: `threat-hunting-output/threat-hunting-bundle.tgz`
- produce: `threat-hunting-output/threat-hunting-findings.csv`
...
- Confirm `threat-hunting-output` contains the report, manifest, and at least one evidence artifact.
...
- Load `threat-intelligence` next if the current findings need deeper validation or formal handoff.
