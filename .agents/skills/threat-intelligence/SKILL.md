...
version: 1.0.0
source: h4cker/threat-intelligence
last_updated: 2026-04-06
reviewed_by: Codex
next_review: 2026-07-05
load_priority: 4
depends_on: [dfir, threat-hunting]
os_support: [kali, ubuntu, parrot]
python_min: 3.8
description: Enrich domains, IPs, accounts, and incident artifacts with defensive context, ownership evidence, and IOC grouping for authorized analysis.
allowed-tools: Read, Glob, Grep, Bash
---
# SKILL: Threat Intelligence

Summary: Enrich domains, IPs, accounts, and incident artifacts with defensive context, ownership evidence, and IOC grouping for authorized analysis.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.

## WHEN TO USE THIS SKILL

- Use when the task involves IOC enrichment, ownership context, or grouping incident clues into a usable intelligence summary
- Use when Codex should connect domains, IPs, accounts, and log findings without drifting into offensive actions
- Use when a defensive investigation needs context before response, hardening, or stakeholder communication
- Use when the output should distinguish confirmed evidence, enrichment context, and unresolved gaps

## KEY TECHNIQUES & TOOLS

### Phase 1: Normalize raw indicators

```bash
mkdir -p threat-intel-output
printf '%s\n' example.com 203.0.113.10 alice@example.com > threat-intel-output/seed-indicators.txt
rg -n "([0-9]{1,3}\\.){3}[0-9]{1,3}|[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}|[A-Za-z0-9.-]+\\.[A-Za-z]{2,}" /path/to/logs 2>/dev/null > threat-intel-output/extracted-indicators.txt || true
```

### Phase 2: Enrich domains with DNS and ownership context

```bash
python3 .agents/scripts/h4cker_dns_ownership.py example.com > threat-intel-output/dns-example.json
dig example.com +short
whois example.com | sed -n '1,40p'
```

### Phase 3: Group incident evidence with JSON pivots

```bash
jq '.summary, .iocs' ir-work/analysis.json 2>/dev/null || true
jq -r '.findings[]? | [.severity, .type, .description] | @tsv' ir-work/analysis.json 2>/dev/null || true
grep -RIn "203.0.113.10\\|example.com\\|alice@example.com" . 2>/dev/null | head -100
```

### Phase 4: Compare indicators against local scope

```bash
getent hosts example.com
grep -RIn "example.com\\|203.0.113.10" config .env* deploy* 2>/dev/null || true
```

### Phase 5: Produce a triage-friendly artifact set

```bash
cp threat-intel-output/seed-indicators.txt threat-intel-output/final-indicators.txt
ls -lh threat-intel-output
```

### Decision rules

- If an indicator cannot be verified from local evidence, label it as unconfirmed enrichment instead of a confirmed finding
- If a domain resolves to third-party infrastructure, separate ownership context from maliciousness claims
- If the same indicator appears across logs, configs, and enrichment output, elevate it for manual review
- If enrichment depends on unavailable tools, keep the raw indicator set and document the missing context step
...
- `Scope` must state:
  - where the indicators came from
  - whether the task was incident-driven or baseline enrichment
  - whether ownership lookup was available

- `Raw Indicators` should group:
  - domains
  - IPs
  - email accounts
  - usernames

- `Enrichment Findings` must include:
  - DNS or ownership context
  - matching local evidence
  - any clustering across artifacts

- `Confirmed vs Unconfirmed` should separate:
  - seen in local evidence
  - seen only in enrichment
  - still requiring validation

- `Artifacts` should list:
  - `threat-intel-output/seed-indicators.txt`
  - `threat-intel-output/extracted-indicators.txt`
  - `threat-intel-output/dns-example.json`
  - `threat-intel-output/final-indicators.txt`

- `Recommended Next Actions` must stay defensive:
  - monitoring updates
  - ownership confirmation
  - log pivots
  - containment review when evidence supports it
...
Use when you need a stable starting point before deeper threat-intelligence review.
```bash
# Create the output directory for this skill
mkdir -p threat-intelligence-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > threat-intelligence-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > threat-intelligence-output/scope-note.txt
```
...
head -30 threat-intelligence-output/artifact-manifest.txt | xargs -r sha256sum > threat-intelligence-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > threat-intelligence-output/review-start-utc.txt
date > threat-intelligence-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > threat-intelligence-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' threat-intelligence-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 threat-intelligence-output/signal-hits.txt | sort | uniq -c | sort -nr > threat-intelligence-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > threat-intelligence-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > threat-intelligence-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' threat-intelligence-output/extension-summary.txt
```
...
git status --short > threat-intelligence-output/git-status.txt 2>/dev/null || true
git diff --stat > threat-intelligence-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > threat-intelligence-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > threat-intelligence-output/threat-intelligence-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > threat-intelligence-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' threat-intelligence-output/threat-intelligence-report-starter.md
```
...
find threat-intelligence-output -maxdepth 2 -type f | sort > threat-intelligence-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf threat-intelligence-output/threat-intelligence-bundle.tgz threat-intelligence-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf threat-intelligence-output/threat-intelligence-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 threat-intelligence-output/signal-hits.txt | sort | uniq -c | sort -nr > threat-intelligence-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' threat-intelligence-output/review-queue.txt > threat-intelligence-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' threat-intelligence-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > threat-intelligence-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> threat-intelligence-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' threat-intelligence-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > threat-intelligence-output/threat-intelligence-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > threat-intelligence-output/threat-intelligence-final.md
# Preview the output files for QA
sed -n '1,20p' threat-intelligence-output/threat-intelligence-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > threat-intelligence-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 threat-intelligence-output/threat-intelligence-bundle.tgz > threat-intelligence-output/threat-intelligence-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > threat-intelligence-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' threat-intelligence-output/review-queue.txt > threat-intelligence-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > threat-intelligence-output/permission-limited-files.txt
```
...
find threat-intelligence-output -maxdepth 2 -type f | sort > threat-intelligence-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > threat-intelligence-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' threat-intelligence-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > threat-intelligence-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > threat-intelligence-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > threat-intelligence-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > threat-intelligence-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > threat-intelligence-output/current-utc.txt
```
...
- produce: `threat-intelligence-output/threat-intelligence-report.md`
- produce: `threat-intelligence-output/output-manifest.txt`
- produce: `threat-intelligence-output/threat-intelligence-bundle.tgz`
- produce: `threat-intelligence-output/threat-intelligence-findings.csv`
...
- Confirm `threat-intelligence-output` contains the report, manifest, and at least one evidence artifact.
...
- Load `dfir` next if the current findings need deeper validation or formal handoff.
