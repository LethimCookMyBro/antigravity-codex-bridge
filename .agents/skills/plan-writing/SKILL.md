...
Summary: Structured task planning with clear breakdowns, dependencies, and verification criteria. Use when implementing features, refactoring, or any multi-step work.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `plan-writing` or the folder's specialized domain
...
```bash
mkdir -p plan-writing-output
pwd > plan-writing-output/pwd.txt
rg --files . | head -200 > plan-writing-output/repo-files.txt
git status --short > plan-writing-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper plan-writing review.
```bash
# Create the output directory for this skill
mkdir -p plan-writing-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > plan-writing-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > plan-writing-output/scope-note.txt
```
...
head -30 plan-writing-output/artifact-manifest.txt | xargs -r sha256sum > plan-writing-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > plan-writing-output/review-start-utc.txt
date > plan-writing-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > plan-writing-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' plan-writing-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 plan-writing-output/signal-hits.txt | sort | uniq -c | sort -nr > plan-writing-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > plan-writing-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > plan-writing-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' plan-writing-output/extension-summary.txt
```
...
git status --short > plan-writing-output/git-status.txt 2>/dev/null || true
git diff --stat > plan-writing-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > plan-writing-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > plan-writing-output/plan-writing-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > plan-writing-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' plan-writing-output/plan-writing-report-starter.md
```
...
find plan-writing-output -maxdepth 2 -type f | sort > plan-writing-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf plan-writing-output/plan-writing-bundle.tgz plan-writing-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf plan-writing-output/plan-writing-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 plan-writing-output/signal-hits.txt | sort | uniq -c | sort -nr > plan-writing-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' plan-writing-output/review-queue.txt > plan-writing-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' plan-writing-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > plan-writing-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> plan-writing-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' plan-writing-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > plan-writing-output/plan-writing-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > plan-writing-output/plan-writing-final.md
# Preview the output files for QA
sed -n '1,20p' plan-writing-output/plan-writing-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > plan-writing-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 plan-writing-output/plan-writing-bundle.tgz > plan-writing-output/plan-writing-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > plan-writing-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' plan-writing-output/review-queue.txt > plan-writing-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > plan-writing-output/permission-limited-files.txt
```
...
find plan-writing-output -maxdepth 2 -type f | sort > plan-writing-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > plan-writing-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' plan-writing-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > plan-writing-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > plan-writing-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > plan-writing-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > plan-writing-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > plan-writing-output/current-utc.txt
```
...
- produce: `plan-writing-output/plan-writing-report.md`
- produce: `plan-writing-output/output-manifest.txt`
- produce: `plan-writing-output/plan-writing-bundle.tgz`
- produce: `plan-writing-output/plan-writing-findings.csv`
...
- Confirm `plan-writing-output` contains the report, manifest, and at least one evidence artifact.
