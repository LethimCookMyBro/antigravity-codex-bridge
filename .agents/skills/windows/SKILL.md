...
description: Review Windows host posture, services, firewall, logs, and scheduled persistence with defensive evidence-first commands.
allowed-tools: Read, Glob, Grep, Bash
---
# SKILL: Windows

Summary: Review Windows host posture, services, firewall, logs, and scheduled persistence with defensive evidence-first commands.
Does NOT cover persistence or credential abuse; stay with defensive host review and hardening.

## WHEN TO USE THIS SKILL

- Use when the task involves a Windows workstation or server that needs defensive review or hardening triage
- Use when Codex should gather service, firewall, log, and startup evidence without improvising exploitation
- Use when the user wants a Windows baseline before deeper incident response or system administration work
- Use when the output should clearly separate confirmed evidence from assumptions

## KEY TECHNIQUES & TOOLS

### Phase 1: Capture host identity and patch context

```bash
powershell.exe -NoProfile -Command "Get-ComputerInfo | Select-Object CsName,WindowsProductName,WindowsVersion,OsBuildNumber"
powershell.exe -NoProfile -Command "Get-HotFix | Sort-Object InstalledOn -Descending | Select-Object -First 15"
```

### Phase 2: Review services, startup, and tasks

```bash
powershell.exe -NoProfile -Command "Get-Service | Where-Object {$_.Status -eq 'Running'} | Sort-Object DisplayName | Select-Object -First 50"
powershell.exe -NoProfile -Command "Get-ScheduledTask | Where-Object {$_.State -ne 'Disabled'} | Select-Object TaskName,TaskPath,State"
powershell.exe -NoProfile -Command "Get-CimInstance Win32_StartupCommand | Select-Object Name,Command,Location"
```

### Phase 3: Inspect network and firewall posture

```bash
powershell.exe -NoProfile -Command "Get-NetTCPConnection -State Listen | Select-Object LocalAddress,LocalPort,OwningProcess"
powershell.exe -NoProfile -Command "Get-NetFirewallProfile | Select-Object Name,Enabled,DefaultInboundAction,DefaultOutboundAction"
powershell.exe -NoProfile -Command "Get-NetFirewallRule | Where-Object {$_.Enabled -eq 'True'} | Select-Object -First 40 DisplayName,Direction,Action"
```

### Phase 4: Review log signals and defender state

```bash
powershell.exe -NoProfile -Command "Get-WinEvent -LogName Security -MaxEvents 50 | Select-Object TimeCreated,Id,ProviderName,LevelDisplayName"
powershell.exe -NoProfile -Command "Get-MpComputerStatus | Select-Object AMServiceEnabled,AntispywareEnabled,AntivirusEnabled,RealTimeProtectionEnabled"
```

### Phase 5: Capture privileges and high-signal artifacts

```bash
powershell.exe -NoProfile -Command "whoami /priv"
powershell.exe -NoProfile -Command "Get-ChildItem 'C:\\ProgramData' -Force | Select-Object -First 40 Name,LastWriteTime"
```

### Decision rules

- If the machine is not Windows or PowerShell remoting is unavailable, stop and report the environment mismatch instead of guessing
- If firewall is disabled or default inbound is permissive, prioritize network hardening before lower-impact cleanup
- If scheduled tasks or startup entries look suspicious, move into containment or DFIR review before baseline tuning
- If Defender is disabled on a non-exempt host, treat that as a primary defensive gap
...
- Produce these sections:
  - `Scope`
  - `Host Baseline`
  - `Services and Startup Findings`
  - `Firewall and Network Findings`
  - `Log and Defender Findings`
  - `Recommended Next Actions`

- `Scope` must state:
  - hostname
  - Windows version if known
  - local vs remote collection context

- `Host Baseline` should include:
  - OS build
  - recent hotfix coverage
  - privilege context used for review

- `Services and Startup Findings` must include:
  - unusual running services
  - scheduled tasks of interest
  - suspicious startup commands

- `Firewall and Network Findings` should include:
  - listener summary
  - firewall profile state
  - rules needing manual review

- `Log and Defender Findings` must separate:
  - confirmed alerts or errors
  - missing telemetry
  - suspicious but unconfirmed signals

- `Recommended Next Actions` must stay defensive:
  - host hardening
  - firewall cleanup
  - service review
  - event-log or DFIR follow-up
...
- Edge case 1: PowerShell remoting is unavailable; use local-only evidence and state the collection gap.
- Edge case 2: the host uses non-English event logs; reference event IDs and fields, not only message text.
...
Use when you need a stable starting point before deeper windows review.
```bash
# Create the output directory for this skill
mkdir -p windows-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > windows-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > windows-output/scope-note.txt
```
...
head -30 windows-output/artifact-manifest.txt | xargs -r sha256sum > windows-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > windows-output/review-start-utc.txt
date > windows-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > windows-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' windows-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 windows-output/signal-hits.txt | sort | uniq -c | sort -nr > windows-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > windows-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > windows-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' windows-output/extension-summary.txt
```
...
git status --short > windows-output/git-status.txt 2>/dev/null || true
git diff --stat > windows-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > windows-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > windows-output/windows-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > windows-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' windows-output/windows-report-starter.md
```
...
find windows-output -maxdepth 2 -type f | sort > windows-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf windows-output/windows-bundle.tgz windows-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf windows-output/windows-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 windows-output/signal-hits.txt | sort | uniq -c | sort -nr > windows-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' windows-output/review-queue.txt > windows-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' windows-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > windows-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> windows-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' windows-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > windows-output/windows-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > windows-output/windows-final.md
# Preview the output files for QA
sed -n '1,20p' windows-output/windows-final.md
```
...
- If the current skill reveals a narrower follow-on task → save artifacts first, then load `linux-hardening`.
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > windows-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 windows-output/windows-bundle.tgz > windows-output/windows-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > windows-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' windows-output/review-queue.txt > windows-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > windows-output/permission-limited-files.txt
```
...
find windows-output -maxdepth 2 -type f | sort > windows-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > windows-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' windows-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > windows-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > windows-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > windows-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > windows-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > windows-output/current-utc.txt
```
...
- produce: `windows-output/windows-report.md`
- produce: `windows-output/output-manifest.txt`
- produce: `windows-output/windows-bundle.tgz`
- produce: `windows-output/windows-findings.csv`
...
## Commands Used
## Artifacts Produced
## Next Steps → load `linux-hardening`
```
...
- Confirm `windows-output` contains the report, manifest, and at least one evidence artifact.
...
- What exact question should the next skill `linux-hardening` answer?
...
- The exact next skill name: `linux-hardening`.
...
## Next: load `linux-hardening` skill

- Load `linux-hardening` next if the current findings need deeper validation or formal handoff.
