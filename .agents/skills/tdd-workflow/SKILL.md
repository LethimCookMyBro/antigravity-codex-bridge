...
Summary: Test-Driven Development workflow principles. RED-GREEN-REFACTOR cycle.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `tdd-workflow` or the folder's specialized domain
...
```bash
mkdir -p tdd-workflow-output
pwd > tdd-workflow-output/pwd.txt
rg --files . | head -200 > tdd-workflow-output/repo-files.txt
git status --short > tdd-workflow-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper tdd-workflow review.
```bash
# Create the output directory for this skill
mkdir -p tdd-workflow-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > tdd-workflow-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > tdd-workflow-output/scope-note.txt
```
...
head -30 tdd-workflow-output/artifact-manifest.txt | xargs -r sha256sum > tdd-workflow-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > tdd-workflow-output/review-start-utc.txt
date > tdd-workflow-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > tdd-workflow-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' tdd-workflow-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 tdd-workflow-output/signal-hits.txt | sort | uniq -c | sort -nr > tdd-workflow-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > tdd-workflow-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > tdd-workflow-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' tdd-workflow-output/extension-summary.txt
```
...
git status --short > tdd-workflow-output/git-status.txt 2>/dev/null || true
git diff --stat > tdd-workflow-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > tdd-workflow-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > tdd-workflow-output/tdd-workflow-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > tdd-workflow-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' tdd-workflow-output/tdd-workflow-report-starter.md
```
...
find tdd-workflow-output -maxdepth 2 -type f | sort > tdd-workflow-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf tdd-workflow-output/tdd-workflow-bundle.tgz tdd-workflow-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf tdd-workflow-output/tdd-workflow-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 tdd-workflow-output/signal-hits.txt | sort | uniq -c | sort -nr > tdd-workflow-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' tdd-workflow-output/review-queue.txt > tdd-workflow-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' tdd-workflow-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > tdd-workflow-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> tdd-workflow-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' tdd-workflow-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > tdd-workflow-output/tdd-workflow-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > tdd-workflow-output/tdd-workflow-final.md
# Preview the output files for QA
sed -n '1,20p' tdd-workflow-output/tdd-workflow-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > tdd-workflow-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 tdd-workflow-output/tdd-workflow-bundle.tgz > tdd-workflow-output/tdd-workflow-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > tdd-workflow-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' tdd-workflow-output/review-queue.txt > tdd-workflow-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > tdd-workflow-output/permission-limited-files.txt
```
...
find tdd-workflow-output -maxdepth 2 -type f | sort > tdd-workflow-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > tdd-workflow-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' tdd-workflow-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > tdd-workflow-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > tdd-workflow-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > tdd-workflow-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > tdd-workflow-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > tdd-workflow-output/current-utc.txt
```
...
- produce: `tdd-workflow-output/tdd-workflow-report.md`
- produce: `tdd-workflow-output/output-manifest.txt`
- produce: `tdd-workflow-output/tdd-workflow-bundle.tgz`
- produce: `tdd-workflow-output/tdd-workflow-findings.csv`
...
- Confirm `tdd-workflow-output` contains the report, manifest, and at least one evidence artifact.
