...
Summary: Game development orchestrator. Routes to platform-specific skills based on project needs.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `game-development` or the folder's specialized domain
...
```bash
mkdir -p game-development-output
pwd > game-development-output/pwd.txt
rg --files . | head -200 > game-development-output/repo-files.txt
git status --short > game-development-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper game-development review.
```bash
# Create the output directory for this skill
mkdir -p game-development-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > game-development-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > game-development-output/scope-note.txt
```
...
head -30 game-development-output/artifact-manifest.txt | xargs -r sha256sum > game-development-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > game-development-output/review-start-utc.txt
date > game-development-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > game-development-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' game-development-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 game-development-output/signal-hits.txt | sort | uniq -c | sort -nr > game-development-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > game-development-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > game-development-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' game-development-output/extension-summary.txt
```
...
git status --short > game-development-output/git-status.txt 2>/dev/null || true
git diff --stat > game-development-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > game-development-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > game-development-output/game-development-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > game-development-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' game-development-output/game-development-report-starter.md
```
...
find game-development-output -maxdepth 2 -type f | sort > game-development-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf game-development-output/game-development-bundle.tgz game-development-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf game-development-output/game-development-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 game-development-output/signal-hits.txt | sort | uniq -c | sort -nr > game-development-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' game-development-output/review-queue.txt > game-development-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' game-development-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > game-development-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> game-development-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' game-development-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > game-development-output/game-development-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > game-development-output/game-development-final.md
# Preview the output files for QA
sed -n '1,20p' game-development-output/game-development-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > game-development-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 game-development-output/game-development-bundle.tgz > game-development-output/game-development-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > game-development-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' game-development-output/review-queue.txt > game-development-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > game-development-output/permission-limited-files.txt
```
...
find game-development-output -maxdepth 2 -type f | sort > game-development-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > game-development-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' game-development-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > game-development-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > game-development-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > game-development-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > game-development-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > game-development-output/current-utc.txt
```
...
- produce: `game-development-output/game-development-report.md`
- produce: `game-development-output/output-manifest.txt`
- produce: `game-development-output/game-development-bundle.tgz`
- produce: `game-development-output/game-development-findings.csv`
...
- Confirm `game-development-output` contains the report, manifest, and at least one evidence artifact.
