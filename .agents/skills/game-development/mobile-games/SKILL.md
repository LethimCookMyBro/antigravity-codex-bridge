...
name: mobile-games
version: 1.0.0
source: codex/game-development/mobile-games
...
- Use when the task clearly matches `mobile-games` or the folder's specialized domain
...
```bash
mkdir -p mobile-games-output
pwd > mobile-games-output/pwd.txt
rg --files . | head -200 > mobile-games-output/repo-files.txt
git status --short > mobile-games-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper mobile-games review.
```bash
# Create the output directory for this skill
mkdir -p mobile-games-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > mobile-games-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > mobile-games-output/scope-note.txt
```
...
head -30 mobile-games-output/artifact-manifest.txt | xargs -r sha256sum > mobile-games-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > mobile-games-output/review-start-utc.txt
date > mobile-games-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > mobile-games-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' mobile-games-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 mobile-games-output/signal-hits.txt | sort | uniq -c | sort -nr > mobile-games-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > mobile-games-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > mobile-games-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' mobile-games-output/extension-summary.txt
```
...
git status --short > mobile-games-output/git-status.txt 2>/dev/null || true
git diff --stat > mobile-games-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > mobile-games-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > mobile-games-output/mobile-games-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > mobile-games-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' mobile-games-output/mobile-games-report-starter.md
```
...
find mobile-games-output -maxdepth 2 -type f | sort > mobile-games-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf mobile-games-output/mobile-games-bundle.tgz mobile-games-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf mobile-games-output/mobile-games-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 mobile-games-output/signal-hits.txt | sort | uniq -c | sort -nr > mobile-games-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' mobile-games-output/review-queue.txt > mobile-games-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' mobile-games-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > mobile-games-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> mobile-games-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' mobile-games-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > mobile-games-output/mobile-games-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > mobile-games-output/mobile-games-final.md
# Preview the output files for QA
sed -n '1,20p' mobile-games-output/mobile-games-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > mobile-games-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 mobile-games-output/mobile-games-bundle.tgz > mobile-games-output/mobile-games-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > mobile-games-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' mobile-games-output/review-queue.txt > mobile-games-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > mobile-games-output/permission-limited-files.txt
```
...
find mobile-games-output -maxdepth 2 -type f | sort > mobile-games-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > mobile-games-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' mobile-games-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > mobile-games-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > mobile-games-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > mobile-games-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > mobile-games-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > mobile-games-output/current-utc.txt
```
...
- produce: `mobile-games-output/mobile-games-report.md`
- produce: `mobile-games-output/output-manifest.txt`
- produce: `mobile-games-output/mobile-games-bundle.tgz`
- produce: `mobile-games-output/mobile-games-findings.csv`
...
- Confirm `mobile-games-output` contains the report, manifest, and at least one evidence artifact.
