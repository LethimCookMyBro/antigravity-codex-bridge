...
name: 2d-games
version: 1.0.0
source: codex/game-development/2d-games
...
- Use when the task clearly matches `2d-games` or the folder's specialized domain
...
```bash
mkdir -p 2d-games-output
pwd > 2d-games-output/pwd.txt
rg --files . | head -200 > 2d-games-output/repo-files.txt
git status --short > 2d-games-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper 2d-games review.
```bash
# Create the output directory for this skill
mkdir -p 2d-games-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > 2d-games-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > 2d-games-output/scope-note.txt
```
...
head -30 2d-games-output/artifact-manifest.txt | xargs -r sha256sum > 2d-games-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > 2d-games-output/review-start-utc.txt
date > 2d-games-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > 2d-games-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' 2d-games-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 2d-games-output/signal-hits.txt | sort | uniq -c | sort -nr > 2d-games-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > 2d-games-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > 2d-games-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' 2d-games-output/extension-summary.txt
```
...
git status --short > 2d-games-output/git-status.txt 2>/dev/null || true
git diff --stat > 2d-games-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > 2d-games-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > 2d-games-output/2d-games-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > 2d-games-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' 2d-games-output/2d-games-report-starter.md
```
...
find 2d-games-output -maxdepth 2 -type f | sort > 2d-games-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf 2d-games-output/2d-games-bundle.tgz 2d-games-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf 2d-games-output/2d-games-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 2d-games-output/signal-hits.txt | sort | uniq -c | sort -nr > 2d-games-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' 2d-games-output/review-queue.txt > 2d-games-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' 2d-games-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > 2d-games-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> 2d-games-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' 2d-games-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > 2d-games-output/2d-games-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > 2d-games-output/2d-games-final.md
# Preview the output files for QA
sed -n '1,20p' 2d-games-output/2d-games-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > 2d-games-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 2d-games-output/2d-games-bundle.tgz > 2d-games-output/2d-games-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > 2d-games-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' 2d-games-output/review-queue.txt > 2d-games-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > 2d-games-output/permission-limited-files.txt
```
...
find 2d-games-output -maxdepth 2 -type f | sort > 2d-games-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > 2d-games-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' 2d-games-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > 2d-games-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > 2d-games-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > 2d-games-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > 2d-games-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > 2d-games-output/current-utc.txt
```
...
- produce: `2d-games-output/2d-games-report.md`
- produce: `2d-games-output/output-manifest.txt`
- produce: `2d-games-output/2d-games-bundle.tgz`
- produce: `2d-games-output/2d-games-findings.csv`
...
- Confirm `2d-games-output` contains the report, manifest, and at least one evidence artifact.
