...
name: game-design
version: 1.0.0
source: codex/game-development/game-design
...
- Use when the task clearly matches `game-design` or the folder's specialized domain
...
```bash
mkdir -p game-design-output
pwd > game-design-output/pwd.txt
rg --files . | head -200 > game-design-output/repo-files.txt
git status --short > game-design-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper game-design review.
```bash
# Create the output directory for this skill
mkdir -p game-design-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > game-design-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > game-design-output/scope-note.txt
```
...
head -30 game-design-output/artifact-manifest.txt | xargs -r sha256sum > game-design-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > game-design-output/review-start-utc.txt
date > game-design-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > game-design-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' game-design-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 game-design-output/signal-hits.txt | sort | uniq -c | sort -nr > game-design-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > game-design-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > game-design-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' game-design-output/extension-summary.txt
```
...
git status --short > game-design-output/git-status.txt 2>/dev/null || true
git diff --stat > game-design-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > game-design-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > game-design-output/game-design-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > game-design-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' game-design-output/game-design-report-starter.md
```
...
find game-design-output -maxdepth 2 -type f | sort > game-design-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf game-design-output/game-design-bundle.tgz game-design-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf game-design-output/game-design-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 game-design-output/signal-hits.txt | sort | uniq -c | sort -nr > game-design-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' game-design-output/review-queue.txt > game-design-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' game-design-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > game-design-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> game-design-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' game-design-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > game-design-output/game-design-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > game-design-output/game-design-final.md
# Preview the output files for QA
sed -n '1,20p' game-design-output/game-design-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > game-design-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 game-design-output/game-design-bundle.tgz > game-design-output/game-design-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > game-design-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' game-design-output/review-queue.txt > game-design-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > game-design-output/permission-limited-files.txt
```
...
find game-design-output -maxdepth 2 -type f | sort > game-design-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > game-design-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' game-design-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > game-design-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > game-design-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > game-design-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > game-design-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > game-design-output/current-utc.txt
```
...
- produce: `game-design-output/game-design-report.md`
- produce: `game-design-output/output-manifest.txt`
- produce: `game-design-output/game-design-bundle.tgz`
- produce: `game-design-output/game-design-findings.csv`
...
- Confirm `game-design-output` contains the report, manifest, and at least one evidence artifact.
