...
name: game-audio
version: 1.0.0
source: codex/game-development/game-audio
...
- Use when the task clearly matches `game-audio` or the folder's specialized domain
...
```bash
mkdir -p game-audio-output
pwd > game-audio-output/pwd.txt
rg --files . | head -200 > game-audio-output/repo-files.txt
git status --short > game-audio-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper game-audio review.
```bash
# Create the output directory for this skill
mkdir -p game-audio-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > game-audio-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > game-audio-output/scope-note.txt
```
...
head -30 game-audio-output/artifact-manifest.txt | xargs -r sha256sum > game-audio-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > game-audio-output/review-start-utc.txt
date > game-audio-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > game-audio-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' game-audio-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 game-audio-output/signal-hits.txt | sort | uniq -c | sort -nr > game-audio-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > game-audio-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > game-audio-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' game-audio-output/extension-summary.txt
```
...
git status --short > game-audio-output/git-status.txt 2>/dev/null || true
git diff --stat > game-audio-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > game-audio-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > game-audio-output/game-audio-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > game-audio-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' game-audio-output/game-audio-report-starter.md
```
...
find game-audio-output -maxdepth 2 -type f | sort > game-audio-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf game-audio-output/game-audio-bundle.tgz game-audio-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf game-audio-output/game-audio-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 game-audio-output/signal-hits.txt | sort | uniq -c | sort -nr > game-audio-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' game-audio-output/review-queue.txt > game-audio-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' game-audio-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > game-audio-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> game-audio-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' game-audio-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > game-audio-output/game-audio-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > game-audio-output/game-audio-final.md
# Preview the output files for QA
sed -n '1,20p' game-audio-output/game-audio-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > game-audio-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 game-audio-output/game-audio-bundle.tgz > game-audio-output/game-audio-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > game-audio-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' game-audio-output/review-queue.txt > game-audio-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > game-audio-output/permission-limited-files.txt
```
...
find game-audio-output -maxdepth 2 -type f | sort > game-audio-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > game-audio-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' game-audio-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > game-audio-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > game-audio-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > game-audio-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > game-audio-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > game-audio-output/current-utc.txt
```
...
- produce: `game-audio-output/game-audio-report.md`
- produce: `game-audio-output/output-manifest.txt`
- produce: `game-audio-output/game-audio-bundle.tgz`
- produce: `game-audio-output/game-audio-findings.csv`
...
- Confirm `game-audio-output` contains the report, manifest, and at least one evidence artifact.
