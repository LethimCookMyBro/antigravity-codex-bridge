...
name: multiplayer
version: 1.0.0
source: codex/game-development/multiplayer
...
- Use when the task clearly matches `multiplayer` or the folder's specialized domain
...
```bash
mkdir -p multiplayer-output
pwd > multiplayer-output/pwd.txt
rg --files . | head -200 > multiplayer-output/repo-files.txt
git status --short > multiplayer-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper multiplayer review.
```bash
# Create the output directory for this skill
mkdir -p multiplayer-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > multiplayer-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > multiplayer-output/scope-note.txt
```
...
head -30 multiplayer-output/artifact-manifest.txt | xargs -r sha256sum > multiplayer-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > multiplayer-output/review-start-utc.txt
date > multiplayer-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > multiplayer-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' multiplayer-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 multiplayer-output/signal-hits.txt | sort | uniq -c | sort -nr > multiplayer-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > multiplayer-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > multiplayer-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' multiplayer-output/extension-summary.txt
```
...
git status --short > multiplayer-output/git-status.txt 2>/dev/null || true
git diff --stat > multiplayer-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > multiplayer-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > multiplayer-output/multiplayer-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > multiplayer-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' multiplayer-output/multiplayer-report-starter.md
```
...
find multiplayer-output -maxdepth 2 -type f | sort > multiplayer-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf multiplayer-output/multiplayer-bundle.tgz multiplayer-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf multiplayer-output/multiplayer-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 multiplayer-output/signal-hits.txt | sort | uniq -c | sort -nr > multiplayer-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' multiplayer-output/review-queue.txt > multiplayer-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' multiplayer-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > multiplayer-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> multiplayer-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' multiplayer-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > multiplayer-output/multiplayer-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > multiplayer-output/multiplayer-final.md
# Preview the output files for QA
sed -n '1,20p' multiplayer-output/multiplayer-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > multiplayer-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 multiplayer-output/multiplayer-bundle.tgz > multiplayer-output/multiplayer-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > multiplayer-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' multiplayer-output/review-queue.txt > multiplayer-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > multiplayer-output/permission-limited-files.txt
```
...
find multiplayer-output -maxdepth 2 -type f | sort > multiplayer-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > multiplayer-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' multiplayer-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > multiplayer-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > multiplayer-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > multiplayer-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > multiplayer-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > multiplayer-output/current-utc.txt
```
...
- produce: `multiplayer-output/multiplayer-report.md`
- produce: `multiplayer-output/output-manifest.txt`
- produce: `multiplayer-output/multiplayer-bundle.tgz`
- produce: `multiplayer-output/multiplayer-findings.csv`
...
- Confirm `multiplayer-output` contains the report, manifest, and at least one evidence artifact.
