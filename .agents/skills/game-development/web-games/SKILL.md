...
name: web-games
version: 1.0.0
source: codex/game-development/web-games
...
- Use when the task clearly matches `web-games` or the folder's specialized domain
...
```bash
mkdir -p web-games-output
pwd > web-games-output/pwd.txt
rg --files . | head -200 > web-games-output/repo-files.txt
git status --short > web-games-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper web-games review.
```bash
# Create the output directory for this skill
mkdir -p web-games-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > web-games-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > web-games-output/scope-note.txt
```
...
head -30 web-games-output/artifact-manifest.txt | xargs -r sha256sum > web-games-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > web-games-output/review-start-utc.txt
date > web-games-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > web-games-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' web-games-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 web-games-output/signal-hits.txt | sort | uniq -c | sort -nr > web-games-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > web-games-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > web-games-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' web-games-output/extension-summary.txt
```
...
git status --short > web-games-output/git-status.txt 2>/dev/null || true
git diff --stat > web-games-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > web-games-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > web-games-output/web-games-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > web-games-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' web-games-output/web-games-report-starter.md
```
...
find web-games-output -maxdepth 2 -type f | sort > web-games-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf web-games-output/web-games-bundle.tgz web-games-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf web-games-output/web-games-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 web-games-output/signal-hits.txt | sort | uniq -c | sort -nr > web-games-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' web-games-output/review-queue.txt > web-games-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' web-games-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > web-games-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> web-games-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' web-games-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > web-games-output/web-games-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > web-games-output/web-games-final.md
# Preview the output files for QA
sed -n '1,20p' web-games-output/web-games-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > web-games-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 web-games-output/web-games-bundle.tgz > web-games-output/web-games-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > web-games-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' web-games-output/review-queue.txt > web-games-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > web-games-output/permission-limited-files.txt
```
...
find web-games-output -maxdepth 2 -type f | sort > web-games-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > web-games-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' web-games-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > web-games-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > web-games-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > web-games-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > web-games-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > web-games-output/current-utc.txt
```
...
- produce: `web-games-output/web-games-report.md`
- produce: `web-games-output/output-manifest.txt`
- produce: `web-games-output/web-games-bundle.tgz`
- produce: `web-games-output/web-games-findings.csv`
...
- Confirm `web-games-output` contains the report, manifest, and at least one evidence artifact.
