...
name: templates
version: 1.0.0
source: codex/app-builder/templates
...
Summary: Project scaffolding templates for new applications. Use when creating new projects from scratch. Contains 12 templates for multiple specific tech stacks.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `templates` or the folder's specialized domain
...
pwd > templates-output/pwd.txt
rg --files . | head -200 > templates-output/repo-files.txt
git status --short > templates-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper templates review.
```bash
# Create the output directory for this skill
mkdir -p templates-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > templates-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > templates-output/scope-note.txt
```
...
head -30 templates-output/artifact-manifest.txt | xargs -r sha256sum > templates-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > templates-output/review-start-utc.txt
date > templates-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > templates-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' templates-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 templates-output/signal-hits.txt | sort | uniq -c | sort -nr > templates-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > templates-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > templates-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' templates-output/extension-summary.txt
```
...
git status --short > templates-output/git-status.txt 2>/dev/null || true
git diff --stat > templates-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > templates-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > templates-output/templates-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > templates-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' templates-output/templates-report-starter.md
```
...
find templates-output -maxdepth 2 -type f | sort > templates-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf templates-output/templates-bundle.tgz templates-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf templates-output/templates-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 templates-output/signal-hits.txt | sort | uniq -c | sort -nr > templates-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' templates-output/review-queue.txt > templates-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' templates-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > templates-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> templates-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' templates-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > templates-output/templates-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > templates-output/templates-final.md
# Preview the output files for QA
sed -n '1,20p' templates-output/templates-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > templates-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 templates-output/templates-bundle.tgz > templates-output/templates-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > templates-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' templates-output/review-queue.txt > templates-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > templates-output/permission-limited-files.txt
```
...
find templates-output -maxdepth 2 -type f | sort > templates-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > templates-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' templates-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > templates-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > templates-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > templates-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > templates-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > templates-output/current-utc.txt
```
...
- produce: `templates-output/templates-report.md`
- produce: `templates-output/output-manifest.txt`
- produce: `templates-output/templates-bundle.tgz`
- produce: `templates-output/templates-findings.csv`
...
- Confirm `templates-output` contains the report, manifest, and at least one evidence artifact.
