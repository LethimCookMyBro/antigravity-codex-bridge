...
version: 1.0.0
source: codex/documentation-templates
...
Summary: Documentation templates and structure guidelines. README, API docs, code comments, and AI-friendly documentation.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `documentation-templates` or the folder's specialized domain
...
```bash
mkdir -p documentation-templates-output
pwd > documentation-templates-output/pwd.txt
rg --files . | head -200 > documentation-templates-output/repo-files.txt
git status --short > documentation-templates-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper documentation-templates review.
```bash
# Create the output directory for this skill
mkdir -p documentation-templates-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > documentation-templates-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > documentation-templates-output/scope-note.txt
```
...
head -30 documentation-templates-output/artifact-manifest.txt | xargs -r sha256sum > documentation-templates-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > documentation-templates-output/review-start-utc.txt
date > documentation-templates-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > documentation-templates-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' documentation-templates-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 documentation-templates-output/signal-hits.txt | sort | uniq -c | sort -nr > documentation-templates-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > documentation-templates-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > documentation-templates-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' documentation-templates-output/extension-summary.txt
```
...
git status --short > documentation-templates-output/git-status.txt 2>/dev/null || true
git diff --stat > documentation-templates-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > documentation-templates-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > documentation-templates-output/documentation-templates-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > documentation-templates-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' documentation-templates-output/documentation-templates-report-starter.md
```
...
find documentation-templates-output -maxdepth 2 -type f | sort > documentation-templates-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf documentation-templates-output/documentation-templates-bundle.tgz documentation-templates-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf documentation-templates-output/documentation-templates-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 documentation-templates-output/signal-hits.txt | sort | uniq -c | sort -nr > documentation-templates-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' documentation-templates-output/review-queue.txt > documentation-templates-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' documentation-templates-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > documentation-templates-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> documentation-templates-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' documentation-templates-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > documentation-templates-output/documentation-templates-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > documentation-templates-output/documentation-templates-final.md
# Preview the output files for QA
sed -n '1,20p' documentation-templates-output/documentation-templates-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > documentation-templates-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 documentation-templates-output/documentation-templates-bundle.tgz > documentation-templates-output/documentation-templates-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > documentation-templates-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' documentation-templates-output/review-queue.txt > documentation-templates-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > documentation-templates-output/permission-limited-files.txt
```
...
find documentation-templates-output -maxdepth 2 -type f | sort > documentation-templates-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > documentation-templates-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' documentation-templates-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > documentation-templates-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > documentation-templates-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > documentation-templates-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > documentation-templates-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > documentation-templates-output/current-utc.txt
```
...
- produce: `documentation-templates-output/documentation-templates-report.md`
- produce: `documentation-templates-output/output-manifest.txt`
- produce: `documentation-templates-output/documentation-templates-bundle.tgz`
- produce: `documentation-templates-output/documentation-templates-findings.csv`
...
- Confirm `documentation-templates-output` contains the report, manifest, and at least one evidence artifact.
