---
skill: metasploit-resources
name: metasploit-resources
version: 1.0.0
source: h4cker/metasploit-resources
...
python_min: 3.8
description: Document framework hygiene, module review, and defensive validation boundaries without operational exploit steps.
allowed-tools: Read, Glob, Grep, Bash
---
# SKILL: Metasploit Resources

Summary: Document framework hygiene, module review, and defensive validation boundaries without operational exploit steps.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.

## WHEN TO USE THIS SKILL

- Use when the task clearly maps to metasploit-resources topics in an owned, authorized, or lab context
...
```bash
mkdir -p metasploit-resources-output
find . -maxdepth 3 -type f | sort | head -100 > metasploit-resources-output/files.txt
ls -la metasploit-resources-output
```

### Phase 2: Review high-signal artifacts

```bash
rg -n "error|alert|warning|TODO|FIXME|password|token|secret" . 2>/dev/null | head -100 > metasploit-resources-output/signals.txt || true
sed -n '1,40p' metasploit-resources-output/signals.txt
```

### Phase 3: Preserve reproducible evidence

```bash
find . -maxdepth 4 -type f | sort > metasploit-resources-output/inventory.txt
wc -l metasploit-resources-output/inventory.txt metasploit-resources-output/signals.txt 2>/dev/null || true
```
...
- `Artifacts` should list:
  - `metasploit-resources-output/files.txt`
  - `metasploit-resources-output/signals.txt`
  - `metasploit-resources-output/inventory.txt`
...
Use when you need a stable starting point before deeper metasploit-resources review.
```bash
# Create the output directory for this skill
mkdir -p metasploit-resources-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > metasploit-resources-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > metasploit-resources-output/scope-note.txt
```
...
head -30 metasploit-resources-output/artifact-manifest.txt | xargs -r sha256sum > metasploit-resources-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > metasploit-resources-output/review-start-utc.txt
date > metasploit-resources-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > metasploit-resources-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' metasploit-resources-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 metasploit-resources-output/signal-hits.txt | sort | uniq -c | sort -nr > metasploit-resources-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > metasploit-resources-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > metasploit-resources-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' metasploit-resources-output/extension-summary.txt
```
...
git status --short > metasploit-resources-output/git-status.txt 2>/dev/null || true
git diff --stat > metasploit-resources-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > metasploit-resources-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > metasploit-resources-output/metasploit-resources-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > metasploit-resources-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' metasploit-resources-output/metasploit-resources-report-starter.md
```
...
find metasploit-resources-output -maxdepth 2 -type f | sort > metasploit-resources-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf metasploit-resources-output/metasploit-resources-bundle.tgz metasploit-resources-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf metasploit-resources-output/metasploit-resources-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 metasploit-resources-output/signal-hits.txt | sort | uniq -c | sort -nr > metasploit-resources-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' metasploit-resources-output/review-queue.txt > metasploit-resources-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' metasploit-resources-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > metasploit-resources-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> metasploit-resources-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' metasploit-resources-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > metasploit-resources-output/metasploit-resources-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > metasploit-resources-output/metasploit-resources-final.md
# Preview the output files for QA
sed -n '1,20p' metasploit-resources-output/metasploit-resources-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > metasploit-resources-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 metasploit-resources-output/metasploit-resources-bundle.tgz > metasploit-resources-output/metasploit-resources-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > metasploit-resources-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' metasploit-resources-output/review-queue.txt > metasploit-resources-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > metasploit-resources-output/permission-limited-files.txt
```
...
find metasploit-resources-output -maxdepth 2 -type f | sort > metasploit-resources-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > metasploit-resources-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' metasploit-resources-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > metasploit-resources-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > metasploit-resources-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > metasploit-resources-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > metasploit-resources-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > metasploit-resources-output/current-utc.txt
```
...
- produce: `metasploit-resources-output/metasploit-resources-report.md`
- produce: `metasploit-resources-output/output-manifest.txt`
- produce: `metasploit-resources-output/metasploit-resources-bundle.tgz`
- produce: `metasploit-resources-output/metasploit-resources-findings.csv`
...
- Confirm `metasploit-resources-output` contains the report, manifest, and at least one evidence artifact.
