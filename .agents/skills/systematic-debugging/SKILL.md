...
version: 1.0.0
source: codex/systematic-debugging
...
Summary: 4-phase systematic debugging methodology with root cause analysis and evidence-based verification. Use when debugging complex issues.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `systematic-debugging` or the folder's specialized domain
...
Use when you need a stable starting point before deeper systematic-debugging review.
```bash
# Create the output directory for this skill
mkdir -p systematic-debugging-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > systematic-debugging-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > systematic-debugging-output/scope-note.txt
```
...
head -30 systematic-debugging-output/artifact-manifest.txt | xargs -r sha256sum > systematic-debugging-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > systematic-debugging-output/review-start-utc.txt
date > systematic-debugging-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > systematic-debugging-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' systematic-debugging-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 systematic-debugging-output/signal-hits.txt | sort | uniq -c | sort -nr > systematic-debugging-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > systematic-debugging-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > systematic-debugging-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' systematic-debugging-output/extension-summary.txt
```
...
git status --short > systematic-debugging-output/git-status.txt 2>/dev/null || true
git diff --stat > systematic-debugging-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > systematic-debugging-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > systematic-debugging-output/systematic-debugging-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > systematic-debugging-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' systematic-debugging-output/systematic-debugging-report-starter.md
```
...
find systematic-debugging-output -maxdepth 2 -type f | sort > systematic-debugging-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf systematic-debugging-output/systematic-debugging-bundle.tgz systematic-debugging-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf systematic-debugging-output/systematic-debugging-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 systematic-debugging-output/signal-hits.txt | sort | uniq -c | sort -nr > systematic-debugging-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' systematic-debugging-output/review-queue.txt > systematic-debugging-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' systematic-debugging-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > systematic-debugging-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> systematic-debugging-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' systematic-debugging-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > systematic-debugging-output/systematic-debugging-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > systematic-debugging-output/systematic-debugging-final.md
# Preview the output files for QA
sed -n '1,20p' systematic-debugging-output/systematic-debugging-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > systematic-debugging-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 systematic-debugging-output/systematic-debugging-bundle.tgz > systematic-debugging-output/systematic-debugging-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > systematic-debugging-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' systematic-debugging-output/review-queue.txt > systematic-debugging-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > systematic-debugging-output/permission-limited-files.txt
```
...
find systematic-debugging-output -maxdepth 2 -type f | sort > systematic-debugging-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > systematic-debugging-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' systematic-debugging-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > systematic-debugging-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > systematic-debugging-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > systematic-debugging-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > systematic-debugging-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > systematic-debugging-output/current-utc.txt
```
...
- produce: `systematic-debugging-output/systematic-debugging-report.md`
- produce: `systematic-debugging-output/output-manifest.txt`
- produce: `systematic-debugging-output/systematic-debugging-bundle.tgz`
- produce: `systematic-debugging-output/systematic-debugging-findings.csv`
...
- Confirm `systematic-debugging-output` contains the report, manifest, and at least one evidence artifact.
