...
version: 1.0.0
source: codex/powershell-windows
...
Summary: PowerShell Windows patterns. Critical pitfalls, operator syntax, error handling.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `powershell-windows` or the folder's specialized domain
...
```bash
mkdir -p powershell-windows-output
pwd > powershell-windows-output/pwd.txt
rg --files . | head -200 > powershell-windows-output/repo-files.txt
git status --short > powershell-windows-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper powershell-windows review.
```bash
# Create the output directory for this skill
mkdir -p powershell-windows-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > powershell-windows-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > powershell-windows-output/scope-note.txt
```
...
head -30 powershell-windows-output/artifact-manifest.txt | xargs -r sha256sum > powershell-windows-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > powershell-windows-output/review-start-utc.txt
date > powershell-windows-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > powershell-windows-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' powershell-windows-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 powershell-windows-output/signal-hits.txt | sort | uniq -c | sort -nr > powershell-windows-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > powershell-windows-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > powershell-windows-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' powershell-windows-output/extension-summary.txt
```
...
git status --short > powershell-windows-output/git-status.txt 2>/dev/null || true
git diff --stat > powershell-windows-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > powershell-windows-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > powershell-windows-output/powershell-windows-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > powershell-windows-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' powershell-windows-output/powershell-windows-report-starter.md
```
...
find powershell-windows-output -maxdepth 2 -type f | sort > powershell-windows-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf powershell-windows-output/powershell-windows-bundle.tgz powershell-windows-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf powershell-windows-output/powershell-windows-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 powershell-windows-output/signal-hits.txt | sort | uniq -c | sort -nr > powershell-windows-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' powershell-windows-output/review-queue.txt > powershell-windows-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' powershell-windows-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > powershell-windows-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> powershell-windows-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' powershell-windows-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > powershell-windows-output/powershell-windows-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > powershell-windows-output/powershell-windows-final.md
# Preview the output files for QA
sed -n '1,20p' powershell-windows-output/powershell-windows-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > powershell-windows-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 powershell-windows-output/powershell-windows-bundle.tgz > powershell-windows-output/powershell-windows-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > powershell-windows-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' powershell-windows-output/review-queue.txt > powershell-windows-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > powershell-windows-output/permission-limited-files.txt
```
...
find powershell-windows-output -maxdepth 2 -type f | sort > powershell-windows-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > powershell-windows-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' powershell-windows-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > powershell-windows-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > powershell-windows-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > powershell-windows-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > powershell-windows-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > powershell-windows-output/current-utc.txt
```
...
- produce: `powershell-windows-output/powershell-windows-report.md`
- produce: `powershell-windows-output/output-manifest.txt`
- produce: `powershell-windows-output/powershell-windows-bundle.tgz`
- produce: `powershell-windows-output/powershell-windows-findings.csv`
...
- Confirm `powershell-windows-output` contains the report, manifest, and at least one evidence artifact.
