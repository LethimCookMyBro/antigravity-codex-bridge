...
description: Public-source intelligence gathering for domains, identities, documents, and relationships with clean evidence and downstream handoff.
allowed-tools: Read, Glob, Grep, Bash
---
# SKILL: OSINT

Summary: Public-source intelligence gathering for domains, identities, documents, and relationships with clean evidence and downstream handoff.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.

## WHEN TO USE THIS SKILL

- Use when the task is public-source discovery rather than service testing
- Use when the user needs identities, domains, leaked documents, org relationships, or public footprint mapping
- Use when Codex should return traceable evidence and not speculation
...
### Phase 1: Domain and document discovery

→ See [shared DNS ownership helper](../_shared/common-commands.md#dns-ownership-helper), then redirect output into `osint/dns.json` when you want a saved artifact.

```bash
mkdir -p osint-output
printf '%s\n' "site:example.com" "site:example.com ext:pdf OR ext:docx OR ext:xlsx" "site:example.com inurl:login OR inurl:admin" > osint-output/search-dorks.txt
cat osint-output/search-dorks.txt
```

### Phase 2: Metadata and relationship extraction

```bash
exiftool public-file.pdf > osint-output/exiftool.txt
sed -n '1,80p' osint-output/exiftool.txt
```

→ See [common DNS baseline](../_shared/common-commands.md#dns-baseline)

### Phase 3: Normalize people, domains, and document clues into one map

```bash
printf '%s\n' "entity,type,source,note" > osint-output/entity-map.csv
printf '%s\n' "example.com,domain,search,main org domain" >> osint-output/entity-map.csv
cat osint-output/entity-map.csv
```

### Decision rules

- If the task asks for direct interaction with accounts, mailboxes, or messaging targets, stop and hand off only to defensive awareness review
- If public documents expose names, hosts, or email formats, record them as evidence and do not infer additional access
- If the same entity appears across DNS, document metadata, and public pages, elevate it in the final map
...
- Produce:
  ...
  - `Entities Identified`
  - `Evidence`
  - `Potential Follow-on Skills`

- `Artifacts` should list:
  - `osint-output/search-dorks.txt`
  - `osint-output/exiftool.txt`
  - `osint-output/entity-map.csv`

## HANDOFF

- Input to `social-engineering`:
  - public roles
...
- Input to `recon`:
  - domains
  - subdomains
  - docs exposing hosts or services

## Quick Mode (< 5 minutes)

- Start with one domain, one company name, or one username.
- Save your search terms before expanding scope.
- Stop once the entity map tells you whether to hand off to `recon` or `social-engineering`.
...
- If the document has no metadata, record that result instead of treating it as tool failure.
- If the same entity appears under multiple brands or subsidiaries, keep those relationships explicit in the entity map.
- Edge case 1: the environment is regional or multilingual; preserve original spellings and translated forms separately.
- Edge case 2: the target uses a country-specific domain or hosting brand; keep ownership context next to the entity.


## Phase Output Map

- Phase 1 output: search terms and discovery scope.
- Phase 2 output: metadata or ownership evidence.
- Phase 3 output: normalized entity map ready for handoff.


## Done When

- Scope is fixed to one org, domain, or identity set.
- At least one saved evidence artifact exists from search, metadata, or ownership work.
- The entity map is good enough for `recon` or `social-engineering` to continue without re-reading sources.

## Technique Depth

### Technique 1: Artifact manifest and scope note
Use when you need a stable starting point before deeper osint review.
```bash
# Create the output directory for this skill
mkdir -p osint-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > osint-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > osint-output/scope-note.txt
```
...
head -30 osint-output/artifact-manifest.txt | xargs -r sha256sum > osint-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > osint-output/review-start-utc.txt
date > osint-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > osint-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' osint-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 osint-output/signal-hits.txt | sort | uniq -c | sort -nr > osint-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > osint-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > osint-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' osint-output/extension-summary.txt
```
...
git status --short > osint-output/git-status.txt 2>/dev/null || true
git diff --stat > osint-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > osint-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > osint-output/osint-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > osint-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' osint-output/osint-report-starter.md
```
...
find osint-output -maxdepth 2 -type f | sort > osint-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf osint-output/osint-bundle.tgz osint-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf osint-output/osint-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 osint-output/signal-hits.txt | sort | uniq -c | sort -nr > osint-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' osint-output/review-queue.txt > osint-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' osint-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > osint-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> osint-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' osint-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > osint-output/osint-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > osint-output/osint-final.md
# Preview the output files for QA
sed -n '1,20p' osint-output/osint-final.md
```
...
- If the current skill reveals a narrower follow-on task → save artifacts first, then load `social-engineering`.
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > osint-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 osint-output/osint-bundle.tgz > osint-output/osint-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > osint-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' osint-output/review-queue.txt > osint-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > osint-output/permission-limited-files.txt
```
...
find osint-output -maxdepth 2 -type f | sort > osint-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > osint-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' osint-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > osint-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > osint-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > osint-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > osint-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > osint-output/current-utc.txt
```
...
- produce: `osint-output/osint-report.md`
- produce: `osint-output/output-manifest.txt`
- produce: `osint-output/osint-bundle.tgz`
- produce: `osint-output/osint-findings.csv`
...
## Commands Used
## Artifacts Produced
## Next Steps → load `social-engineering`
```
...
- Confirm `osint-output` contains the report, manifest, and at least one evidence artifact.
...
- What exact question should the next skill `social-engineering` answer?
...
- The exact next skill name: `social-engineering`.
...
## Next: load `social-engineering` skill

- Load `social-engineering` next if the current findings need deeper validation or formal handoff.
