---
skill: docker-and-k8s-security
name: docker-and-k8s-security
version: 1.0.0
source: h4cker/docker-and-k8s-security
last_updated: 2026-04-06
reviewed_by: Codex
next_review: 2026-07-05
load_priority: 3
depends_on: [devsecops]
os_support: [kali, ubuntu, parrot]
description: Review Docker and Kubernetes configuration, workload exposure, and manifest risk with evidence-first defensive checks.
allowed-tools: Read, Glob, Grep, Bash
---
# SKILL: Docker and K8s Security

Summary: Review Docker and Kubernetes configuration, workload exposure, and manifest risk with evidence-first defensive checks.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.

## WHEN TO USE THIS SKILL

- Use when the task involves container hardening, Kubernetes review, or workload manifest security
- Use when Codex should inspect images, manifests, runtime posture, and privilege settings in owned environments
- Use when the output should distinguish confirmed risky settings from missing context
- Use when a deployment needs a safe pre-release or post-deployment security review

## KEY TECHNIQUES & TOOLS

### Phase 1: Inventory Docker and cluster context

```bash
mkdir -p docker-k8s-output
docker ps --format 'table {{.Names}}\t{{.Image}}\t{{.Ports}}' > docker-k8s-output/docker-ps.txt 2>/dev/null || true
docker images --format 'table {{.Repository}}\t{{.Tag}}\t{{.ID}}' > docker-k8s-output/docker-images.txt 2>/dev/null || true
kubectl get ns > docker-k8s-output/namespaces.txt 2>/dev/null || true
kubectl get pods -A -o wide > docker-k8s-output/pods.txt 2>/dev/null || true
```

### Phase 2: Review manifests and Dockerfiles

```bash
rg -n "privileged: true|hostNetwork: true|hostPID: true|hostIPC: true|allowPrivilegeEscalation: true|runAsNonRoot: false|readOnlyRootFilesystem: false" .
rg -n "imagePullPolicy: Always|latest|NodePort|LoadBalancer" .
rg -n "USER root|ADD http|curl .*\\| sh|chmod 777|sudo " Dockerfile* **/Dockerfile* 2>/dev/null
```

### Phase 3: Inspect workload security context

```bash
kubectl get deploy -A -o yaml > docker-k8s-output/deployments.yaml 2>/dev/null || true
kubectl get podsecuritypolicy -A 2>/dev/null || true
kubectl auth can-i --list > docker-k8s-output/rbac-current.txt 2>/dev/null || true
```

### Phase 4: Inspect runtime exposure and logs

```bash
docker inspect $(docker ps -q) > docker-k8s-output/docker-inspect.json 2>/dev/null || true
kubectl get svc -A -o wide > docker-k8s-output/services.txt 2>/dev/null || true
kubectl logs -n default deploy/example --tail=100 2>/dev/null || true
```

### Phase 5: Correlate evidence

```bash
grep -E "privileged|hostNetwork|runAsNonRoot|readOnlyRootFilesystem" -n docker-k8s-output/deployments.yaml 2>/dev/null || true
jq '.[0].HostConfig // empty' docker-k8s-output/docker-inspect.json 2>/dev/null || true
```

### Decision rules

- If container runtime or cluster access is unavailable, fall back to repo manifest review rather than guessing runtime state
- If workloads run privileged or as root, treat that as a primary finding before discussing lower-impact tuning
- If only static manifests are available, avoid runtime claims and label findings as configuration evidence
- If internet-facing services exist without clear ownership notes, prioritize service exposure mapping and least privilege review

## OUTPUT FORMAT

- Produce these sections:
  - `Scope`
  - `Container and Cluster Inventory`
  - `Manifest Findings`
  - `Runtime Findings`
  - `Artifacts`
...
- `Scope` must state:
  - whether Docker, Kubernetes, or both were reviewed
  - static-manifest vs runtime access level
  - namespace or service boundaries if known

- `Container and Cluster Inventory` should include:
  - running containers
  - namespaces and pods
  - services or externally exposed workloads

- `Manifest Findings` must include:
  - privilege settings
  - image versioning risks
  - Dockerfile or build anti-patterns

- `Runtime Findings` should include:
  - inspect or log evidence
  - RBAC visibility
  - service exposure observations

- `Artifacts` should list:
  - `docker-k8s-output/docker-ps.txt`
  - `docker-k8s-output/pods.txt`
  - `docker-k8s-output/deployments.yaml`
  - `docker-k8s-output/services.txt`

- `Recommended Next Actions` must stay defensive:
  - drop privilege
  - harden runtime policies
  - review service exposure
  - tighten RBAC and image pinning
...
Use when you need a stable starting point before deeper docker-and-k8s-security review.
```bash
# Create the output directory for this skill
mkdir -p docker-and-k8s-security-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > docker-and-k8s-security-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > docker-and-k8s-security-output/scope-note.txt
```
...
head -30 docker-and-k8s-security-output/artifact-manifest.txt | xargs -r sha256sum > docker-and-k8s-security-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > docker-and-k8s-security-output/review-start-utc.txt
date > docker-and-k8s-security-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > docker-and-k8s-security-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' docker-and-k8s-security-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 docker-and-k8s-security-output/signal-hits.txt | sort | uniq -c | sort -nr > docker-and-k8s-security-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > docker-and-k8s-security-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > docker-and-k8s-security-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' docker-and-k8s-security-output/extension-summary.txt
```
...
git status --short > docker-and-k8s-security-output/git-status.txt 2>/dev/null || true
git diff --stat > docker-and-k8s-security-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > docker-and-k8s-security-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > docker-and-k8s-security-output/docker-and-k8s-security-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > docker-and-k8s-security-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' docker-and-k8s-security-output/docker-and-k8s-security-report-starter.md
```
...
find docker-and-k8s-security-output -maxdepth 2 -type f | sort > docker-and-k8s-security-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf docker-and-k8s-security-output/docker-and-k8s-security-bundle.tgz docker-and-k8s-security-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf docker-and-k8s-security-output/docker-and-k8s-security-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 docker-and-k8s-security-output/signal-hits.txt | sort | uniq -c | sort -nr > docker-and-k8s-security-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' docker-and-k8s-security-output/review-queue.txt > docker-and-k8s-security-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' docker-and-k8s-security-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > docker-and-k8s-security-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> docker-and-k8s-security-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' docker-and-k8s-security-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > docker-and-k8s-security-output/docker-and-k8s-security-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > docker-and-k8s-security-output/docker-and-k8s-security-final.md
# Preview the output files for QA
sed -n '1,20p' docker-and-k8s-security-output/docker-and-k8s-security-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > docker-and-k8s-security-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 docker-and-k8s-security-output/docker-and-k8s-security-bundle.tgz > docker-and-k8s-security-output/docker-and-k8s-security-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > docker-and-k8s-security-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' docker-and-k8s-security-output/review-queue.txt > docker-and-k8s-security-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > docker-and-k8s-security-output/permission-limited-files.txt
```
...
find docker-and-k8s-security-output -maxdepth 2 -type f | sort > docker-and-k8s-security-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > docker-and-k8s-security-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' docker-and-k8s-security-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > docker-and-k8s-security-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > docker-and-k8s-security-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > docker-and-k8s-security-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > docker-and-k8s-security-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > docker-and-k8s-security-output/current-utc.txt
```
...
- produce: `docker-and-k8s-security-output/docker-and-k8s-security-report.md`
- produce: `docker-and-k8s-security-output/output-manifest.txt`
- produce: `docker-and-k8s-security-output/docker-and-k8s-security-bundle.tgz`
- produce: `docker-and-k8s-security-output/docker-and-k8s-security-findings.csv`
...
- Confirm `docker-and-k8s-security-output` contains the report, manifest, and at least one evidence artifact.
...
- Load `devsecops` next if the current findings need deeper validation or formal handoff.
