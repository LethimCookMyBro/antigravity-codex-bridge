...
Summary: Web application testing principles. E2E, Playwright, deep audit strategies.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `webapp-testing` or the folder's specialized domain
...
```bash
mkdir -p webapp-testing-output
pwd > webapp-testing-output/pwd.txt
rg --files . | head -200 > webapp-testing-output/repo-files.txt
git status --short > webapp-testing-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper webapp-testing review.
```bash
# Create the output directory for this skill
mkdir -p webapp-testing-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > webapp-testing-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > webapp-testing-output/scope-note.txt
```
...
head -30 webapp-testing-output/artifact-manifest.txt | xargs -r sha256sum > webapp-testing-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > webapp-testing-output/review-start-utc.txt
date > webapp-testing-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > webapp-testing-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' webapp-testing-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 webapp-testing-output/signal-hits.txt | sort | uniq -c | sort -nr > webapp-testing-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > webapp-testing-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > webapp-testing-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' webapp-testing-output/extension-summary.txt
```
...
git status --short > webapp-testing-output/git-status.txt 2>/dev/null || true
git diff --stat > webapp-testing-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > webapp-testing-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > webapp-testing-output/webapp-testing-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > webapp-testing-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' webapp-testing-output/webapp-testing-report-starter.md
```
...
find webapp-testing-output -maxdepth 2 -type f | sort > webapp-testing-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf webapp-testing-output/webapp-testing-bundle.tgz webapp-testing-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf webapp-testing-output/webapp-testing-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 webapp-testing-output/signal-hits.txt | sort | uniq -c | sort -nr > webapp-testing-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' webapp-testing-output/review-queue.txt > webapp-testing-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' webapp-testing-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > webapp-testing-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> webapp-testing-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' webapp-testing-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > webapp-testing-output/webapp-testing-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > webapp-testing-output/webapp-testing-final.md
# Preview the output files for QA
sed -n '1,20p' webapp-testing-output/webapp-testing-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > webapp-testing-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 webapp-testing-output/webapp-testing-bundle.tgz > webapp-testing-output/webapp-testing-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > webapp-testing-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' webapp-testing-output/review-queue.txt > webapp-testing-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > webapp-testing-output/permission-limited-files.txt
```
...
find webapp-testing-output -maxdepth 2 -type f | sort > webapp-testing-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > webapp-testing-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' webapp-testing-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > webapp-testing-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > webapp-testing-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > webapp-testing-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > webapp-testing-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > webapp-testing-output/current-utc.txt
```
...
- produce: `webapp-testing-output/webapp-testing-report.md`
- produce: `webapp-testing-output/output-manifest.txt`
- produce: `webapp-testing-output/webapp-testing-bundle.tgz`
- produce: `webapp-testing-output/webapp-testing-findings.csv`
...
- Confirm `webapp-testing-output` contains the report, manifest, and at least one evidence artifact.
