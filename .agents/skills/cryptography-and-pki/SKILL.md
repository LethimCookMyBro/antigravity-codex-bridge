---
skill: cryptography-and-pki
name: cryptography-and-pki
version: 1.0.0
source: h4cker/cryptography-and-pki
...
python_min: 3.8
description: Review key handling, certificate workflows, TLS posture, and PKI hygiene with practical OpenSSL-based validation and local helper scripts.
allowed-tools: Read, Glob, Grep, Bash
---
# SKILL: Cryptography and PKI

Summary: Review key handling, certificate workflows, TLS posture, and PKI hygiene with practical OpenSSL-based validation and local helper scripts.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.

## WHEN TO USE THIS SKILL

- Use when the task involves certificates, CSRs, key generation, PKI hygiene, or TLS validation
- Use when the user wants practical cryptographic checks with OpenSSL rather than abstract theory
- Use when Codex should review certificate workflows, private-key handling, or service-side TLS evidence
- Use when the work is defensive, operational, and tied to owned or authorized systems
- Use when the answer should distinguish certificate lifecycle issues from actual protocol weaknesses

## KEY TECHNIQUES & TOOLS

### Phase 1: Review key and certificate assets

- Start with inventory:

```bash
find . -maxdepth 2 -type f | grep -E '\\.(pem|crt|cer|key|csr|p12|pfx)$'
file key.pem cert.crt request.csr 2>/dev/null
```

- Inspect key details carefully:
...
openssl x509 -subject -issuer -dates -serial -fingerprint -noout -in cert.crt
```

### Phase 2: Review CSR and issuance workflow

- Create or inspect a CSR:
...
- Use SAN-aware generation patterns when needed:
...
- If the service is internal or lab-only, document whether self-signed issuance is intentional instead of treating it as automatically wrong

### Phase 3: Review certificate and key matching

- Verify that the cert and private key align:
...
- Verify against a CA or chain when available:
...
### Phase 4: Review live TLS services

- Use the local helper for service-side evidence:

→ See [shared TLS audit helpers](../_shared/common-commands.md#tls-audit-helper). This skill extends them with certificate parsing and PKI-specific validation.

- Cross-check with OpenSSL:

```bash
# WARNING: connect only to owned or explicitly authorized hosts
openssl s_client -connect example.com:443 -servername example.com
openssl s_client -connect example.com:443 -showcerts
openssl s_client -connect example.com:443 -status
```

- Extract and inspect the live certificate:

```bash
# WARNING: connect only to owned or explicitly authorized hosts
echo | openssl s_client -connect example.com:443 2>/dev/null | openssl x509 > server_cert.pem
openssl x509 -text -noout -in server_cert.pem
```

### Decision rules

- If the certificate does not match the private key, treat that as a deployment-break or integrity issue before discussing cipher strength
- If the SAN set omits the requested hostname, classify that as the primary certificate defect
- If the certificate is valid but the live service negotiates outdated protocol or weak ciphers, split lifecycle issues from transport issues in the report
- If a PKCS#12 bundle is present, inspect its contents before assuming it contains only the expected leaf certificate and key
- If the environment is internal or lab-only, record trust-model intent explicitly instead of applying public-internet PKI assumptions blindly

### Phase 5: Review key handling and cryptographic hygiene

- Check for weak operational practices:
  - unencrypted private keys where encryption is required
  - keys committed into repos
  - stale or expired certificates
  - missing SANs
  - mixed legacy and modern certificate formats without documentation

- Generate safer modern keys when needed:
...
- Store output artifacts clearly:

```bash
mkdir -p crypto-review
python3 .agents/scripts/h4cker_tls_cert_audit.py example.com > crypto-review/tls-audit.json
openssl x509 -text -noout -in cert.crt > crypto-review/cert-details.txt
```
...
- `Scope` must state:
  - whether the task covered files, live services, or both
  - which hostnames, files, or bundles were inspected

- `Certificate and Key Summary` should include:
  - key type and size if known
  - certificate subject and issuer
  - validity period
  - SAN coverage

- `PKI Findings` must separate:
  - key-handling issues
  - issuance or chain issues
  - lifecycle issues such as expiry

- `TLS Findings` must include:
  - negotiated protocol/cipher evidence
  - hostname validation issues
  - any weak-cipher acceptance found by the helper

- `Artifacts` should list:
  - helper-script output
  - OpenSSL command output
  - extracted PEM, CSR, or bundle-inspection notes

- `Recommended Next Actions` must stay defensive:
  - reissue or renew
  - fix SANs or chain files
  - rotate keys
  - tighten protocol and cipher policy
...
Use when you need a stable starting point before deeper cryptography-and-pki review.
```bash
# Create the output directory for this skill
mkdir -p cryptography-and-pki-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > cryptography-and-pki-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > cryptography-and-pki-output/scope-note.txt
```
...
head -30 cryptography-and-pki-output/artifact-manifest.txt | xargs -r sha256sum > cryptography-and-pki-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > cryptography-and-pki-output/review-start-utc.txt
date > cryptography-and-pki-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > cryptography-and-pki-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' cryptography-and-pki-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 cryptography-and-pki-output/signal-hits.txt | sort | uniq -c | sort -nr > cryptography-and-pki-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > cryptography-and-pki-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > cryptography-and-pki-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' cryptography-and-pki-output/extension-summary.txt
```
...
git status --short > cryptography-and-pki-output/git-status.txt 2>/dev/null || true
git diff --stat > cryptography-and-pki-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > cryptography-and-pki-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > cryptography-and-pki-output/cryptography-and-pki-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > cryptography-and-pki-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' cryptography-and-pki-output/cryptography-and-pki-report-starter.md
```
...
find cryptography-and-pki-output -maxdepth 2 -type f | sort > cryptography-and-pki-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf cryptography-and-pki-output/cryptography-and-pki-bundle.tgz cryptography-and-pki-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf cryptography-and-pki-output/cryptography-and-pki-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 cryptography-and-pki-output/signal-hits.txt | sort | uniq -c | sort -nr > cryptography-and-pki-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' cryptography-and-pki-output/review-queue.txt > cryptography-and-pki-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' cryptography-and-pki-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > cryptography-and-pki-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> cryptography-and-pki-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' cryptography-and-pki-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > cryptography-and-pki-output/cryptography-and-pki-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > cryptography-and-pki-output/cryptography-and-pki-final.md
# Preview the output files for QA
sed -n '1,20p' cryptography-and-pki-output/cryptography-and-pki-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > cryptography-and-pki-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 cryptography-and-pki-output/cryptography-and-pki-bundle.tgz > cryptography-and-pki-output/cryptography-and-pki-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > cryptography-and-pki-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' cryptography-and-pki-output/review-queue.txt > cryptography-and-pki-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > cryptography-and-pki-output/permission-limited-files.txt
```
...
find cryptography-and-pki-output -maxdepth 2 -type f | sort > cryptography-and-pki-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > cryptography-and-pki-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' cryptography-and-pki-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > cryptography-and-pki-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > cryptography-and-pki-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > cryptography-and-pki-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > cryptography-and-pki-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > cryptography-and-pki-output/current-utc.txt
```
...
- produce: `cryptography-and-pki-output/cryptography-and-pki-report.md`
- produce: `cryptography-and-pki-output/output-manifest.txt`
- produce: `cryptography-and-pki-output/cryptography-and-pki-bundle.tgz`
- produce: `cryptography-and-pki-output/cryptography-and-pki-findings.csv`
...
- Confirm `cryptography-and-pki-output` contains the report, manifest, and at least one evidence artifact.
