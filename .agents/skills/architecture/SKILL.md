...
- Use when the task clearly matches `architecture` or the folder's specialized domain
...
pwd > architecture-output/pwd.txt
rg --files . | head -200 > architecture-output/repo-files.txt
git status --short > architecture-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper architecture review.
```bash
# Create the output directory for this skill
mkdir -p architecture-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > architecture-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > architecture-output/scope-note.txt
```
...
head -30 architecture-output/artifact-manifest.txt | xargs -r sha256sum > architecture-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > architecture-output/review-start-utc.txt
date > architecture-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > architecture-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' architecture-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 architecture-output/signal-hits.txt | sort | uniq -c | sort -nr > architecture-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > architecture-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > architecture-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' architecture-output/extension-summary.txt
```
...
git status --short > architecture-output/git-status.txt 2>/dev/null || true
git diff --stat > architecture-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > architecture-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > architecture-output/architecture-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > architecture-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' architecture-output/architecture-report-starter.md
```
...
find architecture-output -maxdepth 2 -type f | sort > architecture-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf architecture-output/architecture-bundle.tgz architecture-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf architecture-output/architecture-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 architecture-output/signal-hits.txt | sort | uniq -c | sort -nr > architecture-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' architecture-output/review-queue.txt > architecture-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' architecture-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > architecture-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> architecture-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' architecture-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > architecture-output/architecture-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > architecture-output/architecture-final.md
# Preview the output files for QA
sed -n '1,20p' architecture-output/architecture-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > architecture-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 architecture-output/architecture-bundle.tgz > architecture-output/architecture-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > architecture-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' architecture-output/review-queue.txt > architecture-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > architecture-output/permission-limited-files.txt
```
...
find architecture-output -maxdepth 2 -type f | sort > architecture-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > architecture-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' architecture-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > architecture-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > architecture-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > architecture-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > architecture-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > architecture-output/current-utc.txt
```
...
- produce: `architecture-output/architecture-report.md`
- produce: `architecture-output/output-manifest.txt`
- produce: `architecture-output/architecture-bundle.tgz`
- produce: `architecture-output/architecture-findings.csv`
...
- Confirm `architecture-output` contains the report, manifest, and at least one evidence artifact.
