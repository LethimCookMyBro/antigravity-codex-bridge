...
description: Build and review software bill of materials evidence, lockfiles, and package manifests for supply-chain visibility in owned projects.
allowed-tools: Read, Glob, Grep, Bash
---
# SKILL: SBOM

Summary: Build and review software bill of materials evidence, lockfiles, and package manifests for supply-chain visibility in owned projects.
Does NOT cover package tampering; focus on inventory, provenance, and dependency risk review.

## WHEN TO USE THIS SKILL

- Use when the user wants package visibility, supply-chain review, or a dependency inventory for an owned project
- Use when Codex should collect manifest and lockfile evidence before recommending remediation
- Use when the task involves provenance, transitive dependency review, or release-package transparency
- Use when the output must be an artifact list plus a concise risk summary

## KEY TECHNIQUES & TOOLS

### Phase 1: Find manifests and lockfiles

```bash
mkdir -p sbom-output
find . -maxdepth 4 \( -name "package.json" -o -name "pnpm-lock.yaml" -o -name "package-lock.json" -o -name "requirements*.txt" -o -name "pyproject.toml" -o -name "poetry.lock" -o -name "go.mod" -o -name "go.sum" -o -name "Cargo.toml" -o -name "Cargo.lock" \) | sort > sbom-output/manifests.txt
cat sbom-output/manifests.txt
```

### Phase 2: Capture ecosystem-specific inventories

```bash
npm ls --all --json > sbom-output/npm-tree.json 2>/dev/null || true
pip freeze > sbom-output/pip-freeze.txt 2>/dev/null || true
cargo tree > sbom-output/cargo-tree.txt 2>/dev/null || true
go list -m all > sbom-output/go-modules.txt 2>/dev/null || true
```

### Phase 3: Generate a modern SBOM when tooling exists

```bash
syft dir:. -o json > sbom-output/syft.json 2>/dev/null || true
syft dir:. -o table > sbom-output/syft.txt 2>/dev/null || true
```

### Phase 4: Inspect risky dependency signals

```bash
rg -n "\"latest\"|\\*|github:|git\\+" package.json pyproject.toml requirements*.txt . 2>/dev/null
rg -n "postinstall|prepare|preinstall" package.json . 2>/dev/null
rg -n "replace |exclude |retract " go.mod Cargo.toml . 2>/dev/null
```

### Phase 5: Normalize evidence for reporting

```bash
wc -l sbom-output/manifests.txt sbom-output/pip-freeze.txt sbom-output/go-modules.txt sbom-output/cargo-tree.txt 2>/dev/null || true
jq '.artifacts[:20]' sbom-output/syft.json 2>/dev/null || true
```

### Decision rules

- If lockfiles are missing, treat reproducibility as a baseline supply-chain gap
- If `syft` is unavailable, fall back to manifest and ecosystem-native inventories instead of failing the workflow
- If install scripts or floating versions appear, prioritize those over broad package-count commentary
- If multiple ecosystems are present, keep findings grouped by ecosystem so remediation stays actionable
...
- `Scope` must state:
  - repo or package boundary reviewed
  - ecosystems detected
  - whether `syft` was available

- `Manifest Inventory` should include:
  - manifest files found
  - lockfiles found or missing
  - ecosystems in scope

- `Dependency Findings` must include:
  - floating or broad version ranges
  - unusual install hooks
  - provenance or pinning concerns

- `SBOM Artifacts` should list:
  - `sbom-output/manifests.txt`
  - `sbom-output/syft.json` when present
  - ecosystem-native outputs such as `npm-tree.json` or `pip-freeze.txt`

- `Recommended Next Actions` must stay defensive:
  - add or refresh lockfiles
  - generate SBOMs in CI
  - review risky install hooks
  - pin or verify high-risk dependencies
...
- Edge case 1: multiple package ecosystems coexist; group findings by ecosystem.
- Edge case 2: lockfiles are intentionally absent; mark reproducibility as a risk, not an implementation bug.
...
Use when you need a stable starting point before deeper sbom review.
```bash
# Create the output directory for this skill
mkdir -p sbom-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > sbom-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > sbom-output/scope-note.txt
```
...
head -30 sbom-output/artifact-manifest.txt | xargs -r sha256sum > sbom-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > sbom-output/review-start-utc.txt
date > sbom-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > sbom-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' sbom-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 sbom-output/signal-hits.txt | sort | uniq -c | sort -nr > sbom-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > sbom-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > sbom-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' sbom-output/extension-summary.txt
```
...
git status --short > sbom-output/git-status.txt 2>/dev/null || true
git diff --stat > sbom-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > sbom-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > sbom-output/sbom-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > sbom-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' sbom-output/sbom-report-starter.md
```
...
find sbom-output -maxdepth 2 -type f | sort > sbom-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf sbom-output/sbom-bundle.tgz sbom-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf sbom-output/sbom-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 sbom-output/signal-hits.txt | sort | uniq -c | sort -nr > sbom-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' sbom-output/review-queue.txt > sbom-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' sbom-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > sbom-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> sbom-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' sbom-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > sbom-output/sbom-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > sbom-output/sbom-final.md
# Preview the output files for QA
sed -n '1,20p' sbom-output/sbom-final.md
```
...
- If the current skill reveals a narrower follow-on task → save artifacts first, then load `docker-and-k8s-security`.
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > sbom-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 sbom-output/sbom-bundle.tgz > sbom-output/sbom-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > sbom-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' sbom-output/review-queue.txt > sbom-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > sbom-output/permission-limited-files.txt
```
...
find sbom-output -maxdepth 2 -type f | sort > sbom-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > sbom-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' sbom-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > sbom-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > sbom-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > sbom-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > sbom-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > sbom-output/current-utc.txt
```
...
- produce: `sbom-output/sbom-report.md`
- produce: `sbom-output/output-manifest.txt`
- produce: `sbom-output/sbom-bundle.tgz`
- produce: `sbom-output/sbom-findings.csv`
...
## Commands Used
## Artifacts Produced
## Next Steps → load `docker-and-k8s-security`
```
...
- Confirm `sbom-output` contains the report, manifest, and at least one evidence artifact.
...
- What exact question should the next skill `docker-and-k8s-security` answer?
...
- The exact next skill name: `docker-and-k8s-security`.
...
## Next: load `docker-and-k8s-security` skill

- Load `docker-and-k8s-security` next if the current findings need deeper validation or formal handoff.
