...
version: 1.0.0
source: h4cker/cracking-passwords
...
python_min: 3.8
description: Defensive password-hash triage. Identify hash types, storage characteristics, and remediation needs without attempting password recovery or providing cracking workflows.
allowed-tools: Read, Glob, Grep, Bash
---
# SKILL: Password Hash Triage

Summary: Defensive password-hash triage. Identify hash types, storage characteristics, and remediation needs without providing cracking workflows.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.

## WHEN TO USE THIS SKILL

- Use when the user provides a password hash and needs it identified or assessed
- Use when a challenge or incident includes stored hashes and Codex must explain format and risk
- Use when the task should stay defensive and non-operational

## INPUT FROM

- raw hash strings
- database dumps already provided by the user
- `capture-the-flag` artifact triage
- `reverse-engineering` extracted strings, helper-script output, or storage-context clues

## KEY TECHNIQUES & TOOLS

### Phase 1: Identify the format

- Classify by prefix and length:
  - `$2a$`, `$2b$`, `$2y$` -> bcrypt
  - `$6$` -> sha512crypt
  - `pbkdf2` markers -> PBKDF2 family
  - 32/40/64 hex only -> raw hash candidate, ambiguous without context

```bash
python3 - <<'PY'
sample = "$2y$10$....................................................."
parts = sample.split("$")
print({"family": "bcrypt" if sample.startswith("$2") else "unknown", "cost": parts[2] if len(parts) > 2 else None})
PY
```

### Phase 2: Assess storage quality
...
```bash
python3 - <<'PY'
samples = ["$2y$10$abcdefghijklmnopqrstuuR4r6b1k0mZ8l5Lx5fM4tQZx2wWn7C", "5f4dcc3b5aa765d61d8327deb882cf99"]
for value in samples:
    family = "bcrypt" if value.startswith("$2") else "raw-hex-candidate"
    print({"hash": value[:12] + "...", "family": family, "length": len(value)})
PY
```

### Decision rules

- Do not provide cracking commands, wordlists, GPU workflows, or bypass instructions
- If the hash is ambiguous, say exactly what context is missing rather than guessing
- If the request is clearly operational cracking, refuse that part and offer detection, migration, or hardening guidance instead

## Fallbacks

- If the hash family is ambiguous, ask for the storage context, application source, or adjacent schema instead of guessing
- If the input contains multiple mixed hash formats, separate them by family before assessing risk
- If the request is a live cracking request, stop and return safe defensive guidance only
...
- Input to `reverse-engineering`:
  - if the hash came from a challenge binary or helper script
- Input to `dfir`:
  - if the hashes appear in evidence and require exposure assessment
...
Use when you need a stable starting point before deeper cracking-passwords review.
```bash
# Create the output directory for this skill
mkdir -p cracking-passwords-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > cracking-passwords-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > cracking-passwords-output/scope-note.txt
```
...
head -30 cracking-passwords-output/artifact-manifest.txt | xargs -r sha256sum > cracking-passwords-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > cracking-passwords-output/review-start-utc.txt
date > cracking-passwords-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > cracking-passwords-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' cracking-passwords-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 cracking-passwords-output/signal-hits.txt | sort | uniq -c | sort -nr > cracking-passwords-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > cracking-passwords-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > cracking-passwords-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' cracking-passwords-output/extension-summary.txt
```
...
git status --short > cracking-passwords-output/git-status.txt 2>/dev/null || true
git diff --stat > cracking-passwords-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > cracking-passwords-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > cracking-passwords-output/cracking-passwords-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > cracking-passwords-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' cracking-passwords-output/cracking-passwords-report-starter.md
```
...
find cracking-passwords-output -maxdepth 2 -type f | sort > cracking-passwords-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf cracking-passwords-output/cracking-passwords-bundle.tgz cracking-passwords-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf cracking-passwords-output/cracking-passwords-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 cracking-passwords-output/signal-hits.txt | sort | uniq -c | sort -nr > cracking-passwords-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' cracking-passwords-output/review-queue.txt > cracking-passwords-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' cracking-passwords-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > cracking-passwords-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> cracking-passwords-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' cracking-passwords-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > cracking-passwords-output/cracking-passwords-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > cracking-passwords-output/cracking-passwords-final.md
# Preview the output files for QA
sed -n '1,20p' cracking-passwords-output/cracking-passwords-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > cracking-passwords-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 cracking-passwords-output/cracking-passwords-bundle.tgz > cracking-passwords-output/cracking-passwords-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > cracking-passwords-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' cracking-passwords-output/review-queue.txt > cracking-passwords-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > cracking-passwords-output/permission-limited-files.txt
```
...
find cracking-passwords-output -maxdepth 2 -type f | sort > cracking-passwords-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > cracking-passwords-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' cracking-passwords-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > cracking-passwords-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > cracking-passwords-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > cracking-passwords-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > cracking-passwords-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > cracking-passwords-output/current-utc.txt
```
...
- produce: `cracking-passwords-output/cracking-passwords-report.md`
- produce: `cracking-passwords-output/output-manifest.txt`
- produce: `cracking-passwords-output/cracking-passwords-bundle.tgz`
- produce: `cracking-passwords-output/cracking-passwords-findings.csv`
...
- Confirm `cracking-passwords-output` contains the report, manifest, and at least one evidence artifact.
