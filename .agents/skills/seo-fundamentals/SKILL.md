...
Summary: SEO fundamentals, E-E-A-T, Core Web Vitals, and Google algorithm principles.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `seo-fundamentals` or the folder's specialized domain
...
```bash
mkdir -p seo-fundamentals-output
pwd > seo-fundamentals-output/pwd.txt
rg --files . | head -200 > seo-fundamentals-output/repo-files.txt
git status --short > seo-fundamentals-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper seo-fundamentals review.
```bash
# Create the output directory for this skill
mkdir -p seo-fundamentals-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > seo-fundamentals-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > seo-fundamentals-output/scope-note.txt
```
...
head -30 seo-fundamentals-output/artifact-manifest.txt | xargs -r sha256sum > seo-fundamentals-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > seo-fundamentals-output/review-start-utc.txt
date > seo-fundamentals-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > seo-fundamentals-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' seo-fundamentals-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 seo-fundamentals-output/signal-hits.txt | sort | uniq -c | sort -nr > seo-fundamentals-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > seo-fundamentals-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > seo-fundamentals-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' seo-fundamentals-output/extension-summary.txt
```
...
git status --short > seo-fundamentals-output/git-status.txt 2>/dev/null || true
git diff --stat > seo-fundamentals-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > seo-fundamentals-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > seo-fundamentals-output/seo-fundamentals-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > seo-fundamentals-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' seo-fundamentals-output/seo-fundamentals-report-starter.md
```
...
find seo-fundamentals-output -maxdepth 2 -type f | sort > seo-fundamentals-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf seo-fundamentals-output/seo-fundamentals-bundle.tgz seo-fundamentals-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf seo-fundamentals-output/seo-fundamentals-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 seo-fundamentals-output/signal-hits.txt | sort | uniq -c | sort -nr > seo-fundamentals-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' seo-fundamentals-output/review-queue.txt > seo-fundamentals-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' seo-fundamentals-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > seo-fundamentals-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> seo-fundamentals-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' seo-fundamentals-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > seo-fundamentals-output/seo-fundamentals-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > seo-fundamentals-output/seo-fundamentals-final.md
# Preview the output files for QA
sed -n '1,20p' seo-fundamentals-output/seo-fundamentals-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > seo-fundamentals-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 seo-fundamentals-output/seo-fundamentals-bundle.tgz > seo-fundamentals-output/seo-fundamentals-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > seo-fundamentals-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' seo-fundamentals-output/review-queue.txt > seo-fundamentals-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > seo-fundamentals-output/permission-limited-files.txt
```
...
find seo-fundamentals-output -maxdepth 2 -type f | sort > seo-fundamentals-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > seo-fundamentals-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' seo-fundamentals-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > seo-fundamentals-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > seo-fundamentals-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > seo-fundamentals-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > seo-fundamentals-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > seo-fundamentals-output/current-utc.txt
```
...
- produce: `seo-fundamentals-output/seo-fundamentals-report.md`
- produce: `seo-fundamentals-output/output-manifest.txt`
- produce: `seo-fundamentals-output/seo-fundamentals-bundle.tgz`
- produce: `seo-fundamentals-output/seo-fundamentals-findings.csv`
...
- Confirm `seo-fundamentals-output` contains the report, manifest, and at least one evidence artifact.
