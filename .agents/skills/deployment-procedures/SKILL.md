...
version: 1.0.0
source: codex/deployment-procedures
...
Summary: Production deployment principles and decision-making. Safe deployment workflows, rollback strategies, and verification. Teaches thinking, not scripts.
Does NOT cover unapproved live-target actions, unauthorized access, or unverified assumptions.
## WHEN TO USE THIS SKILL

- Use when the task clearly matches `deployment-procedures` or the folder's specialized domain
...
```bash
mkdir -p deployment-procedures-output
pwd > deployment-procedures-output/pwd.txt
rg --files . | head -200 > deployment-procedures-output/repo-files.txt
git status --short > deployment-procedures-output/git-status.txt 2>/dev/null || true
```
...
Use when you need a stable starting point before deeper deployment-procedures review.
```bash
# Create the output directory for this skill
mkdir -p deployment-procedures-output
# Save a full file manifest for the current workspace
find . -maxdepth 5 -type f | sort > deployment-procedures-output/artifact-manifest.txt
# Start a short scope note so the reviewer records boundaries first
printf '%s
' 'scope:' 'assumptions:' 'owner:' > deployment-procedures-output/scope-note.txt
```
...
head -30 deployment-procedures-output/artifact-manifest.txt | xargs -r sha256sum > deployment-procedures-output/artifact-hashes.txt
# Save UTC and local timestamps for the review start
date -u > deployment-procedures-output/review-start-utc.txt
date > deployment-procedures-output/review-start-local.txt
```
...
rg -n "error|warn|failed|denied|timeout|exception|secret|token|config" . > deployment-procedures-output/signal-hits.txt 2>/dev/null || true
# Preview the first hits so the next step is explicit
sed -n '1,80p' deployment-procedures-output/signal-hits.txt
# Count which files produced the most hits
cut -d: -f1 deployment-procedures-output/signal-hits.txt | sort | uniq -c | sort -nr > deployment-procedures-output/signal-hit-counts.txt 2>/dev/null || true
```
...
find . -maxdepth 3 -type d | sort > deployment-procedures-output/directory-map.txt
# Capture a file-extension summary for the current tree
find . -maxdepth 5 -type f | sed 's|^.*/||' | awk -F. 'NF>1 {print $NF}' | sort | uniq -c | sort -nr > deployment-procedures-output/extension-summary.txt
# Review the top entries before opening files by hand
sed -n '1,40p' deployment-procedures-output/extension-summary.txt
```
...
git status --short > deployment-procedures-output/git-status.txt 2>/dev/null || true
git diff --stat > deployment-procedures-output/git-diff-stat.txt 2>/dev/null || true
git log --oneline -n 20 > deployment-procedures-output/git-log.txt 2>/dev/null || true
```
...
printf '%s
' '# Summary' '## Scope' '## Evidence' '## Findings' '## Next Actions' > deployment-procedures-output/deployment-procedures-report-starter.md
# Save a structured note file for reviewer observations
printf '%s
' 'observation:' 'confidence:' 'artifact:' 'next_step:' > deployment-procedures-output/review-notes.txt
# Preview the report starter
sed -n '1,40p' deployment-procedures-output/deployment-procedures-report-starter.md
```
...
find deployment-procedures-output -maxdepth 2 -type f | sort > deployment-procedures-output/output-manifest.txt
# Build a compressed handoff bundle
tar -czf deployment-procedures-output/deployment-procedures-bundle.tgz deployment-procedures-output 2>/dev/null || true
# Preview bundle contents for a quick QA pass
tar -tzf deployment-procedures-output/deployment-procedures-bundle.tgz | head -80 2>/dev/null || true
```
...
cut -d: -f1 deployment-procedures-output/signal-hits.txt | sort | uniq -c | sort -nr > deployment-procedures-output/review-queue.txt 2>/dev/null || true
# Save the top ranked entries for the next pass
sed -n '1,30p' deployment-procedures-output/review-queue.txt > deployment-procedures-output/review-queue-top.txt 2>/dev/null || true
# Preview the ranked queue
sed -n '1,30p' deployment-procedures-output/review-queue-top.txt
```
...
find . -maxdepth 4 -type f -readable | sort > deployment-procedures-output/readable-files.txt
# Capture unreadable-path errors separately when possible
find . -maxdepth 4 -type f 2> deployment-procedures-output/find-errors.txt >/dev/null || true
# Review the first readable entries
sed -n '1,50p' deployment-procedures-output/readable-files.txt
```
...
printf '%s
' 'item,severity,detail,artifact,next_step' > deployment-procedures-output/deployment-procedures-findings.csv
# Create a final markdown skeleton with explicit handoff
printf '%s
' '# Summary' '## Findings' '## Commands Used' '## Artifacts Produced' '## Next Steps' > deployment-procedures-output/deployment-procedures-final.md
# Preview the output files for QA
sed -n '1,20p' deployment-procedures-output/deployment-procedures-final.md
```
...
grep -RIn "error\|warn\|failed\|denied\|timeout\|exception\|secret\|token\|config" . > deployment-procedures-output/signal-hits-grep.txt 2>/dev/null || true
```

### ถ้า `sha256sum` is missing:
```bash
# Alternative: use a portable SHA-256 command when available
shasum -a 256 deployment-procedures-output/deployment-procedures-bundle.tgz > deployment-procedures-output/deployment-procedures-bundle.shasum 2>/dev/null || true
```

### ถ้า `git` metadata is unavailable:
```bash
# Alternative: keep a plain file and directory snapshot instead of repo history
find . -maxdepth 4 -type f | sort > deployment-procedures-output/workspace-files.txt
```

### ถ้า logs or source trees are too large:
```bash
# Alternative: review only the first ranked evidence slice
sed -n '1,100p' deployment-procedures-output/review-queue.txt > deployment-procedures-output/review-queue-slice.txt 2>/dev/null || true
```

### ถ้า permission is limited:
```bash
# Alternative: preserve the readable subset and note the boundary explicitly
find . -maxdepth 3 -type f -readable | sort > deployment-procedures-output/permission-limited-files.txt
```
...
find deployment-procedures-output -maxdepth 2 -type f | sort > deployment-procedures-output/quick-artifacts.txt 2>/dev/null || true
# Minimal quick-pass timestamp
date -u > deployment-procedures-output/quick-timestamp.txt
# Minimal quick-pass preview
sed -n '1,40p' deployment-procedures-output/quick-artifacts.txt 2>/dev/null || true
```
...
วิธีจัดการ:
```bash
find . -maxdepth 3 -type f -readable | sort > deployment-procedures-output/readonly-files.txt
```
...
วิธีจัดการ:
```bash
rg --files . | head -500 > deployment-procedures-output/first-500-files.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 6 -type d | sort > deployment-procedures-output/deep-directory-map.txt
```
...
วิธีจัดการ:
```bash
find . -maxdepth 5 -type f | sort > deployment-procedures-output/ungitted-manifest.txt
```
...
วิธีจัดการ:
```bash
date -u > deployment-procedures-output/current-utc.txt
```
...
- produce: `deployment-procedures-output/deployment-procedures-report.md`
- produce: `deployment-procedures-output/output-manifest.txt`
- produce: `deployment-procedures-output/deployment-procedures-bundle.tgz`
- produce: `deployment-procedures-output/deployment-procedures-findings.csv`
...
- Confirm `deployment-procedures-output` contains the report, manifest, and at least one evidence artifact.
